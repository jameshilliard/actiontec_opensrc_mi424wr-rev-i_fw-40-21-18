...
struct sta_info* Ap_get_sta(rtapd *apd, u8 *sa, u8 *apidx)
{
	struct sta_info *s;
...
void Ap_handle_session_timer(void *eloop_ctx, void *timeout_ctx)
{
	...
	memcpy(hdr3->sAddr, apd->own_addr, ETH_ALEN);
	// send deauth
	DBGPRINT(RT_DEBUG_TRACE,"AP_HANDLE_SESSION_TIMER : Send Deauth \n");	  
	if (RT_ioctl(apd, CODE_SET, RT_OID_802_11_RADIUS_DATA, buf, len, sta->ApIdx))
	{
