...
void ieee802_1x_receive(rtapd *rtapd, u8 *sa, u8 *apidx, u8 *buf, size_t len, u16 ethertype)
{
	...
	/* XXX: WinXP sends preauth packets with eapol version 1 */
	if (((ethertype == ETH_P_PAE) && (hdr->version != EAPOL_VERSION)) || 
	    ((ethertype == ETH_P_PRE_AUTH) && (hdr->version != EAPOL_VERSION_2)
	    && (hdr->version != EAPOL_VERSION)))
	{
