...
static MV_CESA_TEST_TRACE    cesaTestTrace[MV_CESA_CHANNELS][MV_CESA_TEST_TRACE_SIZE];
...
static void cesaTestTraceAdd(int type)
{
	...
	for(chan = 0; chan < MV_CESA_CHANNELS; chan++) {
		cesaTestTrace[chan][cesaTestTraceIdx].type = type;
		cesaTestTrace[chan][cesaTestTraceIdx].realCause = MV_REG_READ(MV_CESA_ISR_CAUSE_REG(chan));
		//cesaTestTrace[chan][cesaTestTraceIdx].idmaCause = MV_REG_READ(IDMA_CAUSE_REG(chan));
		cesaTestTrace[chan][cesaTestTraceIdx].resources = cesaReqResources[chan];
		cesaTestTrace[chan][cesaTestTraceIdx].pReqReady = pCesaReqReady[chan];
		cesaTestTrace[chan][cesaTestTraceIdx].pReqEmpty = pCesaReqEmpty[chan];
		cesaTestTrace[chan][cesaTestTraceIdx].pReqProcess = pCesaReqProcess[chan];
		cesaTestTrace[chan][cesaTestTraceIdx].timeStamp = mvCntmrRead(MV_CESA_USE_TIMER_ID);
		cesaTestTraceIdx++;
...
cesa_ocf_process(device_t dev, struct cryptop *crp, int hint)
{
	...
	for(chan = 0; chan < MV_CESA_CHANNELS; chan++)
		...
		if( free_resrc <= MV_CESA_CHANNELS ) {
 ...
 */
static void
cesa_callback(unsigned long chan)
{
	struct cesa_ocf_process *cesa_ocf_cmd = NULL;
	...
	disable_irq(CESA_IRQ);
    	...
    	while(MV_TRUE) {
		 ...
		 /* Get Ready requests */
		spin_lock(&cesa_lock);
		status = mvCesaIfReadyGet((unsigned char)chan, &result);
		...
		if(cesa_ocf_cmd->need_cb) {
			if(debug) {
				mvCesaIfDebugMbuf("DST BUFFER", cesa_ocf_cmd->cesa_cmd.pDst, 0, cesa_ocf_cmd->cesa_cmd.pDst->mbufSize);
			}
			crypto_done(crp);
		}
