	...
	while (bytes_read < size) {
		if ((off + bytes_read < nand_cache_off) ||
			...
			if (nand_read(&nand_info[id->num], nand_cache_off,
						&retlen, nand_cache) != 0 ||
					retlen != NAND_CACHE_SIZE) {
				/* We ignore any NAND read errors at this level (same as Kernel) 
				 * We got data in the buffer and hopefully some or all of it is good
				 * Dual image logic + U-Boot CRC header will take carre of any corruped data
				 * NAND driver will report ECC erros/statistics so no print here
				 */
#if 0
