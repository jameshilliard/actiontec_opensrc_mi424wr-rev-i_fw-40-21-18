...
int nand_burn_uboot_cmd(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	printf("Erasing 0x%x - 0x%x: ",CONFIG_SYS_MONITOR_BASE, CONFIG_SYS_MONITOR_BASE + CONFIG_SYS_MONITOR_LEN);
	{
	    ...
	    opts.offset = CONFIG_SYS_MONITOR_BASE;
	    opts.length = CONFIG_SYS_MONITOR_LEN;
	    ret = nand_erase_opts(nand, &opts);
	    printf("\t[Done]\n");
	}

	printf("Writing image to NAND:");
	ret = nand_write_skip_bad(nand, MV_NBOOT_BASE, (uint64_t*)&len, (u_char*)load_addr);
#else
	printf("Erasing 0x%x - 0x%x: ",CONFIG_SYS_MONITOR_BASE, CONFIG_SYS_MONITOR_BASE + CONFIG_SYS_MONITOR_LEN);
...
static int do_mii_dump(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
    int i;
    MV_U32 page = 0;
    MV_U32 phy = 0;
    ...
    if (argc > 1)
	phy = strtol(argv[1], NULL, 16);
    if (argc > 2)
	page = strtol(argv[2], NULL, 16);

    mvEthPhyRegWrite(phy, 0x16, page);
    for (i = 0; i < 32; i++)
    {
	mvEthPhyRegRead(phy, i, &val);
	printf("[%2x:%2x:%2x] = %x\n", phy, page, i, val);
    }	
    mvEthPhyRegWrite(phy, 0x16, 0x0);

    return 0;
}

U_BOOT_CMD(mii_dump, 4, 0, do_mii_dump, "Dump mii registers", "");
