...
#define ATM_INIT	_IOW('a',ATMIOC_ITF+17,struct atmif_sioc)
					/* start interface */
#define ATM_SHUTDOWN	_IOW('a',ATMIOC_ITF+18,struct atmif_sioc)
					/* stop interface */
#define ATM_SHOWTIME_POLL	_IOW('a',ATMIOC_ITF+19,struct atmif_sioc)
					/* check sync status */
...
struct atmdev_ops { /* only send is required */
	...
	int (*may_send)(struct atm_vcc *vcc, unsigned int size);
	struct module *owner;
};
...
static inline int atm_may_send(struct atm_vcc *vcc,unsigned int size)
{
	...
	(!vcc->dev->ops->may_send || vcc->dev->ops->may_send(vcc, size));
}
