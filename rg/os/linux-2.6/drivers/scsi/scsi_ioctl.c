 ...
 */
static int scsi_ioctl_get_model(struct scsi_device *sdev, struct scsi_vendor_model __user *arg)
{
	...
	memset(&model, 0, sizeof(model));
        ...
        /* vendor */
	len = sizeof(model.vendor) - 1;

	if (len > sdev->inquiry_len)
	    len = sdev->inquiry_len;
	
	strncpy(model.vendor, sdev->inquiry, len);
	model.vendor[len] = 0;

	/* model */
	len = sizeof(model.model) - 1;
	strncpy(model.model, sdev->model, len);
	model.model[len] = 0;

	/* revision */
	len = sizeof(model.rev) - 1;
	strncpy(model.rev, sdev->rev, len);
	model.rev[len] = 0;

	/* host name */
	len = sizeof(model.host) - 1;
	strncpy(model.host, sdev->host->hostt->name, len);
	model.host[len] = 0;
...
int scsi_ioctl(struct scsi_device *sdev, int cmd, void __user *arg)
{
	...
	case SCSI_IOCTL_STOP_UNIT:
        ...
        case SCSI_IOCTL_GET_MODEL:
                return scsi_ioctl_get_model(sdev, arg);
