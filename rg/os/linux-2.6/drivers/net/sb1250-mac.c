...
#define SBMAC_MORE_RX_JOB 0x1
#define SBMAC_MORE_TX_JOB 0x2
	unsigned long more_job;
};
...
static int sbdma_tx_process(struct sbmac_softc *sc,sbmacdma_t *d);
...
static int sbdma_rx_process(struct sbmac_softc *sc,sbmacdma_t *d)
{
	...
	for (count = SBMAC_MAX_RXDESCR / 8; count; count--) {
		/*
 ...
 *  	   nothing
 ********************************************************************* */
static void sbmac_intr_action(struct net_device *dev)
{
	...
	uint64_t isr;
		 ...
		 */
	if (sc->more_job & SBMAC_MORE_TX_JOB ||
		isr & (M_MAC_INT_CHANNEL << S_MAC_TX_CH0))
	{
		if (sbdma_tx_process(sc, &(sc->sbm_txdma)))
			sc->more_job |= SBMAC_MORE_TX_JOB;
		else
			sc->more_job &= ~SBMAC_MORE_TX_JOB;
		}
	...
	if (sc->more_job & SBMAC_MORE_RX_JOB ||
		isr & (M_MAC_INT_CHANNEL << S_MAC_RX_CH0))
	{
		if (sbdma_rx_process(sc, &(sc->sbm_rxdma)))
			sc->more_job |= SBMAC_MORE_RX_JOB;
		else
			sc->more_job &= ~SBMAC_MORE_RX_JOB;
	}
}
...
static irqreturn_t sbmac_intr(int irq,void *dev_instance,struct pt_regs *rgs)
{
    ...
    tasklet_hi_schedule(&sc->tasklet);
...
static void sbmac_tasklet_run(unsigned long dev_id)
{
    struct net_device *dev = (struct net_device *)dev_id;
    struct sbmac_softc *sc = netdev_priv(dev);

    sbmac_intr_action(dev);
    if (!sc->more_job)
	    enable_irq(dev->irq);
    else
	    tasklet_hi_schedule(&sc->tasklet);
...
static int sbmac_init(struct net_device *dev, int idx)
{
	...
	tasklet_init(&sc->tasklet, sbmac_tasklet_run, (unsigned long)dev);
	sc->more_job = 0;
...
#ifdef CONFIG_SIBYTE_COLORADO
	/* MAC1 is connected to the BCM5398 switch, so no PHY polling required */
	if (!strcmp(s->sbm_dev->name, "eth1"))
	{
		s->sbm_phys[0] = 0xff;
		printk(KERN_INFO "%s: switch, PHY polling disabled, "
			"assuming 1000FDx\n", dev->name);
		return 0;
	}
#endif
