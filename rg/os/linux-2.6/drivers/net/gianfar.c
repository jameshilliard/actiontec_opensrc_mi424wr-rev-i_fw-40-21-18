...
static inline void gfar_skb_cache_init(struct gfar_private *priv)
{
    skb_cache_end(priv->skb_cache_handle);
    priv->skb_cache_handle = skb_cache_start(priv->rx_buffer_size +
	RXBUF_ALIGNMENT + 16);
}
...
static inline struct sk_buff *gfar_alloc_skb(struct gfar_private *priv)
{
    struct sk_buff *skb;

    skb = skb_cache_alloc(priv->skb_cache_handle);
    if (skb)
	skb_reserve(skb, 16);
    return skb;
}
...
struct sk_buff * gfar_new_skb(struct net_device *dev, struct rxbd8 *bdp)
{
	...
	while ((!skb) && timeout--)
	{
	    if (priv->skb_cache_handle)
		skb = gfar_alloc_skb(priv);
	    else
