...
static int vsc7385_init(struct gfar_mii_info *mii_info)
{
	/** FIXME: HARDCODED MII INFO **/
	mii_info->duplex = 1;
	mii_info->speed = 1000;
	mii_info->link = 1;
	mii_info->interrupts = MII_INTERRUPT_DISABLED;
	mii_info->advertising = 0;
	mii_info->autoneg = 0;

	return 0;
}

static int vsc7385_suspend(struct gfar_mii_info *mii_info)
{
	/** FIXME: HARDCODED MII INFO **/
	mii_info->duplex = 0;
	mii_info->speed = 0;
...
static int vsc7385_config_aneg(struct gfar_mii_info *mii_info)
{
	/** FIXME: HARDCODED MII INFO **/
	mii_info->advertising = 0;
	mii_info->autoneg = 0;
...
static int vsc7385_read_status(struct gfar_mii_info *mii_info)
{
	/** FIXME **/
	return 0;
}

static int vsc7385_ack_interrupt(struct gfar_mii_info *mii_info)
{
	/** FIXME **/
	return 0;
}

static int vsc7385_config_intr(struct gfar_mii_info *mii_info)
{
	/** FIXME: HARDCODED MII INFO **/
	mii_info->interrupts = MII_INTERRUPT_DISABLED;
	return 0;
...
static int vsc7385_close(struct gfar_mii_info *mii_info)
{
	/** FIXME: HARDCODED MII INFO **/
	mii_info->duplex = 0;
	mii_info->speed = 0;
...
static int vsc8201_init(struct gfar_mii_info *mii_info)
{
	u16 epcr;

	epcr = phy_read(mii_info, MII_CIS8201_EPCR);
	epcr &= ~EPCR_MODE_MASK;

	epcr |= EPCR_GMII_MODE;

	phy_write(mii_info, MII_CIS8201_EPCR, epcr);
...
#ifdef CONFIG_MPC8349_ITX
static struct phy_info phy_info_vsc7385 = {
	.phy_id		= 0x7385,
	.phy_id_mask	= 0xffff,
	.features	= MII_GBIT_FEATURES,
	.name		= "VSC7385 Gigabit Ethernet",
	.init		= &vsc7385_init,
	.suspend	= &vsc7385_suspend,
	.config_aneg	= &vsc7385_config_aneg,
	.read_status	= &vsc7385_read_status,
	.ack_interrupt	= &vsc7385_ack_interrupt,
	.config_intr	= &vsc7385_config_intr,
	.close		= &vsc7385_close,
};

static struct phy_info phy_info_vsc8201 = {
	.phy_id		= 0x000fc410,
	.name		= "VSC8201 Gigabit Ethernet",
