				  ...
				  const char __user *uargs)
{
	...
	for (i = 0; i < hdr->e_shnum; i++) {
		void *dest;
		char *secname = secstrings + sechdrs[i].sh_name;
		...
		if (!strcmp(secname, ".data"))
		    data = sechdrs[i].sh_addr;
		else if (!strcmp(secname, ".text"))
		    text = sechdrs[i].sh_addr;
		else if (!strcmp(secname, ".bss"))
		    bss = sechdrs[i].sh_addr;
		else if (!strcmp(secname, ".main_flow"))
		    main_flow = sechdrs[i].sh_addr;
	}

	if (text)
	    printk("insmod: add-symbol-file build/debug/%s.o %#lx", mod->name, text);
	else
	    printk("insmod: no text section for module %s", mod->name);

	if (main_flow)
	    printk(" -s .main_flow %#lx", main_flow);
	if (data)
	    printk(" -s .data %#lx", data);
	if (bss)
	    printk(" -s .bss %#lx", bss);
	printk("\n");
