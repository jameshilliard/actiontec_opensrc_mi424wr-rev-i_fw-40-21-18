					       ...
					       unsigned short VLAN_ID)
{
	...
	/* Reset DSCP remarking to -1 for any CoS (which means "don't touch") */
	vlan_dev_reset_all_ingress_dscp(new_dev);
#endif
...
#ifdef CONFIG_RG_VLAN_DSCP
	case RESET_ALL_VLAN_INGRESS_DSCP_CMD:
		{
			struct net_device *dev = dev_get_by_name(args.device1);
			if (!dev)
				return -EINVAL;
			err = vlan_dev_reset_all_ingress_dscp(dev);
			dev_put(dev);
		}
		break;

	case SET_VLAN_INGRESS_DSCP_CMD:
		err = vlan_dev_set_ingress_dscp(args.device1,
						args.u.dscp,
						args.vlan_qos);
		break;
#endif
