...
static inline void sco_recv_frame(struct sco_conn *conn, struct sk_buff *skb)
{
	...
	if (conn->is_loopback_mode)
	{
		if (!hci_send_sco(conn->hcon, skb))
			return;
	}
...
static int sco_sock_ioctl(struct socket *sock, unsigned int cmd, unsigned long arg)
{
	int err = -EINVAL;
	struct sco_loopback_mode_req lm;
	struct sock *sk = sock->sk;
	struct sco_conn *conn;
	...
	switch (cmd) {
	case SCOGETLOOPBACKMODE:
		if (!sk)
			return -EBADFD;
		lock_sock(sk);
		if (!(conn = sco_pi(sk)->conn))
		{
			err = -EBADFD;
			goto ErrReleaseSk;
		}
		lm.enabled = conn->is_loopback_mode;
		if (copy_to_user(argp, &lm, sizeof(lm)))
		{
			err = -EFAULT;
			goto ErrReleaseSk;
		}
	...
	case SCOSETLOOPBACKMODE:
		...
		if (copy_from_user(&lm, argp, sizeof(lm)))
		{
			err = -EFAULT;
			goto ErrReleaseSk;
		}
		conn->is_loopback_mode = lm.enabled;
		release_sock(sk);
		break;
