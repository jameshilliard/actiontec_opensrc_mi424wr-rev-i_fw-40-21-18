...
struct {
	...
	[e_vc_brdg] = { sizeof(vc_brdg), vc_brdg, 1 },
	[e_llc_brdg] = { sizeof(llc_brdg), llc_brdg, 1 },
	[e_vc_rtd] = { 0, NULL, 0 },
	[e_llc_rtd] = { sizeof(llc_rtd), llc_rtd, 0 }
};
...
static int br2684_xmit_vcc(struct sk_buff *skb, struct br2684_dev *brdev,
	struct br2684_vcc *brvcc, int *dropped)
{
	struct atm_vcc *atmvcc;
...
static int br2684_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	/* XXX Control (AF_UNSPEC) packets coming from if_ppp are marked with
	 * priority 7 (highest). This should be changed when/if extra flags
	 * are added to sk_buff. */
	else if (skb->priority == 7 &&
	    !atm_may_send(brvcc->atmvcc, skb->truesize))
	{
	    ret = NETDEV_TX_BUSY;
	}
	else if ((ret = br2684_xmit_vcc(skb, brdev, brvcc, &dropped)) == 
	    NETDEV_TX_OK && dropped) 
	{
		/*
...
static void br2684_push(struct atm_vcc *atmvcc, struct sk_buff *skb)
{
	...
	if (skb->len < plen + (br2684_hdrs[brvcc->encaps].is_eth ? ETH_HLEN : 0)) {
		brdev->stats.rx_errors++;
	...
	skb_pull(skb, plen);
	skb->mac.raw = ((char *) (skb->data)) - ETH_HLEN;
	if (br2684_hdrs[brvcc->encaps].is_eth)
...
static void br2684_set_eth_encaps(struct net_device *dev, int is_eth)
{
	if (is_eth)
	{
		dev->rebuild_header = eth_rebuild_header;
		dev->set_mac_address = br2684_mac_addr;
		dev->header_cache_update= eth_header_cache_update;
		dev->type = ARPHRD_ETHER;
#ifdef FASTER_VERSION
		dev->hard_header = br2684_header;
		dev->hard_header_cache = br2684_header_cache;
		dev->hard_header_len = sizeof(llc_oui_pid_pad) + ETH_HLEN;
#else
...
*/
	...
	br2684_set_eth_encaps(net_dev, br2684_hdrs[brvcc->encaps].is_eth);
...
static void br2684_setup(struct net_device *netdev)
{
	...
	br2684_set_eth_encaps(netdev, 1);
...
static int br2684_destroy(void __user *arg)
{
	...
	if (!(net_dev = br2684_find_dev(&ifs)))
		return -ENODEV;
	brdev = BRPRIV(net_dev);
...
static int br2684_seq_show(struct seq_file *seq, void *v)
{
	...
	list_for_each_entry(brvcc, &brdev->brvccs, brvccs) {
		seq_printf(seq, "  vcc %d.%d.%d: encaps=%d"
#ifndef FASTER_VERSION
				    ", failed copies %u/%u"
#endif /* FASTER_VERSION */
