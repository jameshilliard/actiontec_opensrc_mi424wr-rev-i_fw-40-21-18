 ...
 *		The IP to API glue.
 *		
 * Version:	$Id: ip_sockglue.c,v 1.1.1.1.4.1.52.1 2013/04/26 01:27:18 athill Exp $
 *
 * Authors:	see ip.c
 *
 * Fixes:
...
int ip_setsockopt(struct sock *sk, int level, int optname, char __user *optval, int optlen)
{
	...
	if (optname >= MRT_BASE && optname <= (MRT_BASE + IGMP_PROXY_END))
		return ip_mroute_setsockopt(sk,optname,optval,optlen);
#endif
#ifdef CONFIG_RG_IGMP_PROXY
	if (optname >= IGMP_PROXY_BASE && optname <= IGMP_PROXY_BASE + IGMP_PROXY_END)
