...
static DECLARE_MUTEX(ipsec_ioctl_mutex);
static int (*ipsec_ioctl_hook)(unsigned long arg) = NULL;

void ipsec_ioctl_hook_set(int (*hook)(unsigned long))
{
    down(&ipsec_ioctl_mutex);
    ipsec_ioctl_hook = hook;
    up(&ipsec_ioctl_mutex);
}
EXPORT_SYMBOL(ipsec_ioctl_hook_set);
