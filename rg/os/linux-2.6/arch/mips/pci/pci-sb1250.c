...
{
#if CONFIG_SIBYTE_COLORADO
	/*
	 * The NEC USB chip requires 3 interrupts: INTA, INTB and INTC.
	 * miniPCI card with this chip requires some / all interrupts to be
	 * shared between two miniPCI interrupts: INTA and INTB.
	 * IM800A card allows to connect both INTA and INTC to miniPCI INTA
	 * signal, so we are doing a fixup here to reflect this workaround.
	 */
	if (dev->vendor == PCI_VENDOR_ID_NEC && 
		dev->device == PCI_DEVICE_ID_NEC_USB20)
	{
		int irq;

		if (slot == 5 && pin == 3)
			irq = 56;
		else if (slot == 8 && pin == 3)
			irq = 57;
		else
			irq = dev->irq;

		printk("PCI: fixing IM800 NEC USB 2.0 irq line %d->%d\n",
			dev->irq, irq);
