...
static int  HardwareInit(PADAPTER_STRUCT Adapter, int set_default_mac);
...
static __inline void ResetDma(PADAPTER_STRUCT Adapter)
{
	...
	ks8695_ChipInit(Adapter, FALSE, 0);
...
/* #endif */
	...
	if (0 == cards_found) {
		...
		memcpy(DI.stMacStation, "\0\x10\xa1\0\x10\x01", ETH_LENGTH_OF_ADDRESS);

	} else if (1 == cards_found) {
		/* for LAN */
		DI.usDMAId = DMA_LAN;
		DI.nOffset = DMA_LAN;
		...
		memcpy(DI.stMacStation, "\0\x10\xa1\0\x10\x02", ETH_LENGTH_OF_ADDRESS);

#if	!defined(CONFIG_ARCH_KS8695P) && !defined(KS8695X)
	} else if (2 == cards_found) {
		/* for HPNA */
		DI.usDMAId = DMA_HPNA;
		DI.nOffset = DMA_HPNA;
		...
		DI.uIntShift = 18;
		memcpy(DI.stMacStation, "\0\x10\xa1\0\x10\x03", ETH_LENGTH_OF_ADDRESS);
#endif
	} else {
...
static int HardwareInit(PADAPTER_STRUCT Adapter, int set_default_mac)
{
	...
	if (!ks8695_ChipInit(Adapter, TRUE, set_default_mac)) {
...
#ifdef	DEBUG_THIS
					DRV_INFO("%s> port=%d, DSCPEnable=%d, "
					    "IngressPriority802_1P=%d, IngressPriority=%d,"
					    "EgressPriority=%d, IngressTagRemoval=%d, "
					    "StormProtection=%d", __FUNCTION__, uPort,
					    DPI[uPort].byIngressPriorityTOS,
					    DPI[uPort].byIngressPriority802_1P, 
					    DPI[uPort].byIngressPriority,
					    DPI[uPort].byEgressPriority, bRemoval,
					    DPI[uPort].byStormProtection);
#endif
					swConfigurePort(Adapter, uPort);
...
static int ks8695_icache_lock2(void *icache_start, void *icache_end)
{
	...
	__asm__(
#if 0
		/* XXX: ADRL command conflict with -ffunction-sections, since
		 * the symbols are not on the same sections. Removing for now,
		 * since this function isn't in use */
	        "ADRL	r0, ks8695_isr\n"		/* compile complains if icache_start is given instead */
		"ADRL	r1, ks8695_isre\n"		/* compile complains if icache_end is given instead */
#endif
		"MOV	r2, %0\n"
