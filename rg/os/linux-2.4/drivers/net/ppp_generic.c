...
/* We don't want 'trivial' data to keep the ppp up when on demand.
 * Filter out igmp, rip and icmp reply packets.
 */
static int openrg_active_pkt(struct sk_buff *skb, int proto, int is_out)
{
    if (PPP_IP != proto || !skb->nh.iph)
	return 1;

    /* incoming packets are not considered active */
    if (!is_out)
	return 0;
    ...
    {
    case IPPROTO_ICMP:
	/* ignore icmp reply packets */
	skb->h.icmph = (struct icmphdr *)
	    ((char *)skb->nh.iph + skb->nh.iph->ihl * 4); 
	if (ICMP_ECHOREPLY == skb->h.icmph->type)
	    return 0;
	break;
    case IPPROTO_UDP:
	/* ignore rip packets */
	skb->h.uh = (struct udphdr *)
	    ((char *)skb->nh.iph + skb->nh.iph->ihl * 4);
	if (htons(520) == skb->h.uh->source || htons(520) == skb->h.uh->dest)
	    return 0;
	break;
    case IPPROTO_IGMP:
	/* ignore igmp packets */
			 ...
			 size_t count, loff_t *ppos)
{
	...
	/* zero the network header to prevent access by lower level function
	 * in PPP driver.  see B8649 */
	skb->nh.iph = NULL;
...
ppp_send_frame(struct ppp *ppp, struct sk_buff *skb)
{
	...
	if (proto < 0x8000) {
		...
		*skb_push(skb, 2) = 1;
		...
		if (openrg_active_pkt(skb, proto, 1))
		    ppp->last_xmit = jiffies;
#endif /* CONFIG_PPP_FILTER */
	}
...
ppp_receive_nonmp_frame(struct ppp *ppp, struct sk_buff *skb)
{
	...
	if (npi < 0) {
		...
		*skb_push(skb, 2) = 0;
		...
		skb_pull(skb, 2);
#else
		skb->nh.iph = skb->data + 2;
		if (openrg_active_pkt(skb, proto, 0))
		    ppp->last_recv = jiffies;
#endif /* CONFIG_PPP_FILTER */
