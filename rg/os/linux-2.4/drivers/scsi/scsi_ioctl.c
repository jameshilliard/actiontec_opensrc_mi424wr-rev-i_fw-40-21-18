...
/*
 * The scsi_ioctl_get_model() function places into arg vendor name, model
 * and revision number of SCSI device
 */
static int scsi_ioctl_get_model(Scsi_Device *dev, struct scsi_vendor_model *arg)
{
	struct scsi_vendor_model model;
	
	/* Scsi_Device (vendor, model, rev) doesn't contain closing 0 */
	strncpy(model.vendor, dev->vendor, sizeof(dev->vendor));
	model.vendor[sizeof(model.vendor) - 1] = 0;
	strncpy(model.model, dev->model, sizeof(dev->model));
	model.model[sizeof(model.model) - 1] = 0;
	strncpy(model.rev, dev->rev, sizeof(dev->rev));
	model.rev[sizeof(model.rev) - 1] = 0;

	/* Get host name */
	strncpy(model.host, dev->host->hostt->name, sizeof(model.host));
	model.host[sizeof(model.host) - 1] = 0;

	return copy_to_user(arg, &model, sizeof(model));
}
...
int scsi_ioctl(Scsi_Device * dev, int cmd, void *arg)
{
	...
	case SCSI_IOCTL_STOP_UNIT:
        ...
        case SCSI_IOCTL_GET_PCI:
                ...
                break;
	case SCSI_IOCTL_GET_MODEL:
		return scsi_ioctl_get_model(dev, arg);
		break;
	default:
