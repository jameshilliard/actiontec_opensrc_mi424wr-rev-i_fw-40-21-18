...
static int debug_kmalloc_proc(char *page, char **start, off_t off,
				 int count, int *eof, void *data)
{
    kmalloc_show_history(kmalloc_get_label());
    return 0;
}

static ssize_t debug_kmalloc_proc_write(struct file * file, const char *buf,
			     size_t count, loff_t *ppos)
{
    char l;
    int label;
    if (copy_from_user(&l, buf, 1))
	return -EFAULT;
    label = (ATOL(l) >= 0 && ATOL(l) <= 9) ? ATOL(l) : -1;
    kmalloc_show_history(label);
    return count;
}

static ssize_t kmalloc_debug_read_label(char *page, char **start, off_t off,
				 int count, int *eof, void *data)
{
    int len = 0;
    proc_sprintf(page, &off, &len, "%d\n", kmalloc_get_label());
    return proc_calc_metrics(page, start, off, count, eof, len);
}

static ssize_t kmalloc_debug_write_label(struct file * file, const char *buf,
			     size_t count, loff_t *ppos)
{
    char l;
    if (copy_from_user(&l, buf, 1))
    ...
    if (ATOL(l) >= 0 && ATOL(l) <= 9)
    {
	kmalloc_set_label(ATOL(l));
	printk("New label is %d\n", ATOL(l));
    }
    return count;
}


static ssize_t kmalloc_debug_test_read(char *page, char **start, off_t off,
				 int count, int *eof, void *data)
{
    int len = 0;
    proc_sprintf(page, &off, &len, "Usage:\n");
    proc_sprintf(page, &off, &len, "echo test_number > kmalloc_debug_test\n");
    proc_sprintf(page, &off, &len, "Tests:\n");
    proc_sprintf(page, &off, &len, "1) Memory over run\n");
    proc_sprintf(page, &off, &len, "2) Freeing wrong pointer\n");
    proc_sprintf(page, &off, &len, "3) Freeing pointer twice\n");
    return proc_calc_metrics(page, start, off, count, eof, len);
}

static ssize_t kmalloc_debug_test_write(struct file * file, const char *buf,
			     size_t count, loff_t *ppos)
{
    char test;
    if (copy_from_user(&test, buf, 1))
	return -EFAULT;

    if (ATOL(test) >= 1 && ATOL(test) <= 3)
	kmalloc_debug_test(ATOL(test));
    else
	printk("Wrong test number\n");
    return count;
}
...
#endif
#ifdef CONFIG_RG_DEBUG_KMALLOC
	entry = create_proc_read_entry("kmalloc_report", 0, NULL,
	    debug_kmalloc_proc, NULL);
	if (entry)
	    entry->write_proc = debug_kmalloc_proc_write;
	entry = create_proc_read_entry("kmalloc_debug_label", 0, NULL,
	    kmalloc_debug_read_label ,NULL);
	if (entry)
	    entry->write_proc = kmalloc_debug_write_label;
	entry = create_proc_read_entry("kmalloc_debug_test", 0, NULL,
	    kmalloc_debug_test_read, NULL);
	if (entry)
	    entry->write_proc = kmalloc_debug_test_write;
#endif
#ifdef CONFIG_MODULES
