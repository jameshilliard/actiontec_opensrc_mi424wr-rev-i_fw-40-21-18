...
/* __modrd_start and __modrd_end are defined in arch/<arch>/ld.script.in */
extern char __modrd_start, __modrd_end;

static struct buffer_head bh;
static inline struct buffer_head * jsb_bread(struct super_block *sb, int block)
{
#define is_modfs(sb) (!(sb->s_type->fs_flags & FS_REQUIRES_DEV))
    if (is_modfs(sb))
    {
	bh.b_data = &__modrd_start + sb->s_blocksize * block;
	return &bh;
    }
    return sb_bread(sb, block);
}

static inline void jbrelse(struct buffer_head *buf)
{
    if (buf != &bh) /* This is not MOD_FS. Note - you may want to change it */
	return brelse(buf);
}

#define sb_bread jsb_bread
#define brelse jbrelse

static void romfs_putsuper(struct super_block *sb)
{
    extern void free_area(unsigned long addr, unsigned long end, char *s);

    free_area(&__modrd_start, &__modrd_end, "modfs");
}
...
static struct super_operations romfs_ops = {
	...
	statfs:		romfs_statfs,
#ifdef CONFIG_RG_MODFS
	put_super:	romfs_putsuper,
#endif
};

static DECLARE_FSTYPE_DEV(romfs_fs_type, "romfs", romfs_read_super);
#ifdef CONFIG_RG_MODFS
static DECLARE_FSTYPE(modfs_fs_type, "modfs", romfs_read_super, 0);
#endif

static int __init init_romfs_fs(void)
{
	int i = register_filesystem(&romfs_fs_type);
	int j = 0;

#ifdef CONFIG_RG_MODFS
	j = register_filesystem(&modfs_fs_type);
#endif
	return i | j;
}
...
#ifdef CONFIG_RG_MODFS
	unregister_filesystem(&modfs_fs_type);
#endif
}
