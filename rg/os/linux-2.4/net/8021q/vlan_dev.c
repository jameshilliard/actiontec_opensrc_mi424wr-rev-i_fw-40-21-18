...
#ifdef CONFIG_RG_VLAN_DSCP

#define IPHDR_TOS(data) (((struct iphdr *)(data))->tos)
#define IPHDR_CSUM(data) (((struct iphdr *)(data))->check)

static void vlan_dscp_remark(struct sk_buff *skb, struct net_device *dev)
{
	struct vlan_hdr *vhdr = (struct vlan_hdr *)skb->data;
	unsigned short vtci = ntohs(vhdr->h_vlan_TCI);
	unsigned char *payload = skb->mac.raw + VLAN_ETH_HLEN;
	int new_dscp;
	unsigned int l;
	...
	new_dscp = VLAN_DEV_INFO(dev)->ingress_dscp_map[(vtci >> 13) & 0x7];

	/* -1 stands for "don't touch" */
	if (new_dscp == -1 || new_dscp == IPHDR_TOS(payload))
		return;

	/* Change DSCP and fix IP header checksum */
	l = ntohs(IPHDR_CSUM(payload)) + IPHDR_TOS(payload) - new_dscp;
	l = (l >> 16) + (l & 0xffff);
	l = l & 0xffff;
        IPHDR_CSUM(payload) = htons(l);
	IPHDR_TOS(payload) = new_dscp;
}
#endif
...
#ifdef CONFIG_RG_VLAN_DSCP
	/* Remark the DSCP value in the IP header according to the VLAN ID */
	vlan_dscp_remark(skb, skb->dev);
#endif
...
#ifdef CONFIG_RG_VLAN_DSCP
	/* Remark the DSCP value in the IP header according to the VLAN ID */
	vlan_dscp_remark(__skb, __skb->dev);
#endif
...
int vlan_dev_reset_all_ingress_dscp(struct net_device *dev)
{
	...
	for (i = 0; i < 8; i++)
		VLAN_DEV_INFO(dev)->ingress_dscp_map[i] = -1;
...
int vlan_dev_set_ingress_dscp(char *dev_name, int dscp, short vlan_prio)
{
	...
	VLAN_DEV_INFO(dev)->ingress_dscp_map[vlan_prio & 0x7] = dscp;
	dev_put(dev);
	return 0;
}
#endif
