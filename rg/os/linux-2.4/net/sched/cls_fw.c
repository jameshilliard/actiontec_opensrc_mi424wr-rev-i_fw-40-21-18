...
/* Also defined in XXX pkg/firewall/config/qos.c */
#define RG_MAX_QOS_CLASS_ID_BITS 16
#define RG_MAX_QOS_CLASS_ID_MASK ((1 << RG_MAX_QOS_CLASS_ID_BITS) - 1)
			  ...
			  struct tcf_result *res)
{
	...
	extern struct Qdisc_ops ingress_qdisc_ops; /* CONFIG_NET_SCH_INGRESS */
	u32 id = skb->nfmark;
	/* Mask according to RX/TX bitfield */
	if (tp->q->ops != &ingress_qdisc_ops)
	    id >>= RG_MAX_QOS_CLASS_ID_BITS;
	id &= RG_MAX_QOS_CLASS_ID_MASK;
#else
	u32 id = 0;
#endif
