...
#define NUM2PRINT 50
	...
	DPRINTK("br2684: skb: %s\n", buf);
}
#else
#define skb_debug(skb)	do {} while (0)
#endif

static unsigned char vc_brdg[] = { 0x00, 0x00};
static unsigned char llc_brdg[] =
        { 0xAA, 0xAA, 0x03, 0x00, 0x80, 0xC2, 0x00, 0x07, 0x00, 0x00 };
static unsigned char llc_rtd[] =
...
/* Order of entries must match the order of the br2684_encaps enum */
static struct br2684_hdr_t br2684_hdrs[4] = {
	{ sizeof(vc_brdg), vc_brdg },
	{ sizeof(llc_brdg), llc_brdg },
	{ 0, NULL },
	{ sizeof(llc_rtd), llc_rtd }
};

#define PADLEN	(2)

enum br2684_encaps {
	e_vc_brdg  = BR2684_ENCAPS_VC_BRDG,
	e_llc_brdg = BR2684_ENCAPS_LLC_BRDG,
	e_vc_rtd  = BR2684_ENCAPS_VC_RTD,
	e_llc_rtd = BR2684_ENCAPS_LLC_RTD,
};
	...
	struct br2684_vcc *brvcc)
{
	 ...
	 */
#else
	int minheadroom = br2684_hdrs[brvcc->encaps].size;
	...
	if (br2684_hdrs[brvcc->encaps].hdr)
	{
		memcpy(skb->data, br2684_hdrs[brvcc->encaps].hdr,
			br2684_hdrs[brvcc->encaps].size);
		if (brvcc->encaps == e_llc_rtd)
		{
			memcpy(skb->data + br2684_hdrs[brvcc->encaps].size - 
				sizeof(skb->protocol), &skb->protocol,
				sizeof(skb->protocol));
		}
	}
#endif /* FASTER_VERSION */
...
static int br2684_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	if (brvcc == NULL) {
		printk("br2684: no vcc attached to dev %s, freeing skb\n", dev->name);
		brdev->stats.tx_errors++;
		brdev->stats.tx_carrier_errors++;
		/* netif_stop_queue(dev); */
		dev_kfree_skb(skb);
		read_unlock(&devs_lock);
		/* used to be :	return -EUNATCH;
		 * BUT we do not want the packet requeued after it is freed!
		 */
		return 0;
	}
...
static void br2684_push(struct atm_vcc *atmvcc, struct sk_buff *skb)
{
	...
	int cmp_len = 0;
	int plen = br2684_hdrs[brvcc->encaps].size;
	int add_eth_hdr = 0;
	...
	if (brvcc->encaps==e_llc_brdg || brvcc->encaps==e_vc_brdg)
		add_eth_hdr = 1;

	skb_debug(skb);
	atm_return(atmvcc, skb->truesize);
	DPRINTK("skb from brdev %p\n", brdev);
	if (brvcc->encaps == e_llc_brdg) /* no need to check FCS validity */
	        cmp_len = br2684_hdrs[brvcc->encaps].size - 3;
	else if (brvcc->encaps == e_llc_rtd) /* no need to check ether-type */
	        cmp_len = br2684_hdrs[brvcc->encaps].size - 2;
	else if (brvcc->encaps == e_vc_brdg)
	        cmp_len = br2684_hdrs[brvcc->encaps].size;
	if (cmp_len &&
	    memcmp(skb->data, br2684_hdrs[brvcc->encaps].hdr, cmp_len)) {
	        brdev->stats.rx_errors++;
	        dev_kfree_skb(skb);
	        return;
	}
	if (skb->len < plen + add_eth_hdr * ETH_HLEN) {
		brdev->stats.rx_errors++;
	...
	else if (brvcc->encaps == e_llc_rtd)
		skb->protocol = ((u16 *) skb->data)[-1];
	else
...
/* routed protocols mode do not require ethernet header encapsulations */
static void br2684_reset_eth_encaps(struct br2684_vcc *brvcc)
{
	struct net_device *dev = &brvcc->brdev->net_dev;
...
static void br2684_set_eth_encaps(struct br2684_dev *brdev)
{
	struct net_device *dev = &brdev->net_dev;
	...
	brdev->net_dev.hard_header_len = sizeof(llc_oui_pid_pad) +
	    ETH_HLEN; /* 10 + 14 */
#endif
}
...
*/
	...
	if (brdev == NULL) {
		DPRINTK("br2684: tried to attach to non-existant device\n");
		err = -ENXIO;
		goto error;
	}
	if (atmvcc->push == NULL) {
	...
	if (be.fcs_in != BR2684_FCSIN_NO || be.fcs_out != BR2684_FCSOUT_NO ||
	    ...
	    (be.encaps != e_vc_brdg && be.encaps != e_llc_brdg &&
	    be.encaps != e_vc_rtd && be.encaps != e_llc_rtd) ||
	    be.min_size != 0) {
		err = -EINVAL;
		goto error;
	}
	...
	if (be.encaps == e_vc_rtd || be.encaps == e_llc_rtd)
		br2684_reset_eth_encaps(brvcc);
	else
		br2684_set_eth_encaps(brvcc->brdev);
	...
	while ((skb = skb_dequeue(&copy))) {
	    	if(!skb->dev)
		{
		    printk("Warn: Skb's hanging on recv queue while vcc is"
			    " registering\n");
		    dev_kfree_skb(skb);
		    err = -EFAULT;
		    goto error;
...
static int br2684_destroy(unsigned long arg)
{
	struct br2684_if_spec ifs;
	struct br2684_dev *brdev;
    	unsigned long flags;    

	if (copy_from_user(&ifs, (void *)arg, sizeof(ifs)))
		return -EFAULT;

	if (!(brdev = br2684_find_dev(&ifs)))
		return -ENODEV;
