...
static int bnep_session(void *arg)
{
...
#if defined(CONFIG_RG_BLUETOOTH) && defined(CONFIG_HOTPLUG)
	if (sk_disconnected)
	{
		net_run_sbin_hotplug(dev, "remove");
	...
	/* kfree(s);  XXX - removed for bridge workaround */
	return 0;
}
...
/* this code assumes bnep_session_sem is taken */
static void bnep_session_del_conn(struct bnep_session *s)
{
...
int bnep_netdev_del_conn(struct net_device *dev)
{
	...
	s = (struct bnep_session *)dev->priv;
	if (s)
		bnep_session_del_conn(s);
	else
		err = -ENOENT;
