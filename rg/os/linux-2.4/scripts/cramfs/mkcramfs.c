...
static unsigned int blksize_bit = 0;
...
static int opt_compression = CRAMFS_FLAG_COMP_METHOD_GZIP;
...
static void usage(int status)
{
	...
	fprintf(stream, "usage: %s [-h] [-e edition] [-i file] [-n name] dirname outfile\n"
	    ...
	    " -c	   compression method. valid values: none gzip\n"
	    "		   default: gzip\n"
...
static int compress_data(char *dest, unsigned long *dest_len,
    char *source, unsigned long src_len, int level)
{
    switch (opt_compression)
    {
    case CRAMFS_FLAG_COMP_METHOD_GZIP:
	return compress2(dest, dest_len, source, src_len, level);
    case CRAMFS_FLAG_COMP_METHOD_LZMA:
	return lzma_encode(dest, dest_len, source, src_len) == 0 ? Z_OK :
	    Z_DATA_ERROR;
