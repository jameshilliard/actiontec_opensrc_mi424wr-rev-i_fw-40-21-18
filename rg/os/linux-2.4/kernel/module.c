...
/*
 * Initialize a module. now has two modes of operation, signaled by the
 * MOD_KERN_MEM flag. It means that insmod wrote the module directly to
 * kernel memory (to ALL memory areas including fast ram), and we MUST avoid
 * any copy_from_user and other obsolete validity checks. If this flag is not
 * lit, regular behaviour is maintained.
 */
...
sys_init_module(const char *name_user, struct module *mod_user)
{
	...
	if (mod_user->flags & MOD_KERN_MEM)
	    mod_user_size = mod_user->size_of_struct;
	else if ((error = get_user(mod_user_size, &mod_user->size_of_struct))
	    != 0)
	{
		goto err1;
	}
	...
	if (mod->flags & ~(MOD_AUTOCLEAN | MOD_KERN_MEM)) {
