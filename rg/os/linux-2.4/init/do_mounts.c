...
#ifndef CONFIG_RG_NONCOMPRESSED_USERMODE_IMAGE
...
static int copy_dir_to_root_recursive(char *src_root, char *path)
{
    int fd, count;
    struct stat stbuf;
    struct dirent *dirent;
    char *src_name, *dst_name, *tmpname, *buf;
    int ret = 0;
 
    tmpname = kmalloc(PATH_MAX_SIZE, GFP_KERNEL);
    src_name = kmalloc(PATH_MAX_SIZE, GFP_KERNEL);
    dst_name = kmalloc(PATH_MAX_SIZE, GFP_KERNEL);
    buf = kmalloc(BUF_SIZE, GFP_KERNEL);

    if (!tmpname || !src_name || !dst_name || !buf)
    {
	printk("Error allocating buffers\n");
	ret = -ENOMEM;
	goto Exit;
    }

    sprintf(tmpname, "%s/%s", src_root, path);

    fd = sys_open(tmpname, O_RDONLY, 0);
    if (fd < 0)
    {
	printk("Error opening file %s\n", tmpname);
	goto Exit;
    }
    while ((count = sys_getdents(fd, (void *)buf, BUF_SIZE)))
    {
	if (count < 0)
	{
	    ret = count;
	    printk("Error in sys_getdents %s (errno = %d)\n", tmpname, ret);
	    goto Exit;
	}
	...
	while (count)
	{
	    ...
	    {
		sprintf(src_name, "%s/%s", tmpname, dirent->d_name);
		if ((ret = sys_newlstat(src_name, &stbuf)))
		{
		    ret = -1;
		    printk("Error in stat %s (errno = %d)\n", src_name, ret);
		    goto Exit;
		}

		sprintf(dst_name, "%s/%s", path, dirent->d_name);
		/* Remove leading slashes (this is all created under the
		 * current directory (usually /root). */
		for (pdst = dst_name; *pdst == '/'; pdst++);

		switch (stbuf.st_mode & S_IFMT) {
		case S_IFDIR:
		    /* Directory */
		    if ((ret = sys_mkdir(pdst, stbuf.st_mode)))
		    {
			printk("Error in mkdir %s (errno = %d)\n",
			    pdst, ret);
			goto Exit;
		    }
		    if ((ret =
			copy_dir_to_root_recursive(src_root, pdst)))
		    {
			goto Exit;
		    }
		    break;
		case S_IFLNK:
		    /* Symbolic link */
		    ret = sys_readlink(src_name, src_name, PATH_MAX_SIZE);
		    if (ret < 0)
		    {
			printk("Error in readlink %s (errno = %d)\n",
			    src_name, ret);
			goto Exit;
		    }
		    ...
		    if ((ret = sys_symlink(src_name, pdst)))
		    {
			printk("Error in symlink %s->%s (errno = %d)\n",
			    pdst, src_name, ret);
			goto Exit;
		    }
		    break;
		...
		case S_IFSOCK:
		    /* Special file */
		    if ((ret =
			sys_mknod(pdst, stbuf.st_mode, stbuf.st_rdev)))
		    {
			printk("Error in mknod %s (errno = %d)\n", pdst,
			    ret);
			goto Exit;
		    }
		    break;
		case S_IFREG:
			...
			from_fd = sys_open(src_name, O_RDONLY, 0);
			if (from_fd < 0)
			{
			    ret = -1;
			...
			to_fd = sys_open(pdst, O_WRONLY|O_CREAT, stbuf.st_mode);
			if (to_fd < 0)
			{
			    sys_close(from_fd);
			    ret = -1;
			...
			while ((nbytes = read(from_fd, tmp_buf, 64)))
			{
			    if (nbytes < 0)
			    {
				ret = nbytes;
				printk("Error reading from %s (errno = %d)\n",
				    src_name, ret);
				goto Exit;
			    }
			    if (nbytes != (ret = write(to_fd, tmp_buf, nbytes)))
			    {
				ret = -1;
				printk("Error writing to %s (errno = %d)\n",
				    pdst, ret);
				goto Exit;
			    }
			}
...
static int prepare_non_block_rootfs(void)
{
#ifdef CONFIG_RG_INITFS_CRAMFS
    	mm_segment_t fs;
	
	sys_mkdir("/init_cramfs", 0700);
	if (do_mount("cramfs", "/init_cramfs", "cramfs", MS_RDONLY, (void *)1))
	    printk("Warning: unable to mount initial cramfs\n");
	else
	    printk("Mount of initial cramfs successful\n");

	fs = get_fs();
	set_fs(KERNEL_DS);
	copy_dir_to_root_recursive("/init_cramfs", "");
	set_fs(fs);
	sys_umount("/init_cramfs", 0);
	sys_rmdir("/init_cramfs");
	return 0;
#endif
}

int __init mount_tmpfs_root(void)
{
	int err;
	int tmpfs_root_mountflags = root_mountflags;
	
	tmpfs_root_mountflags |= MS_VERBOSE;
	/* Best not mount this read-only, since the next function
	 * plans to write to this... */
	tmpfs_root_mountflags &= ~MS_RDONLY;

	/* Mount the tmpfs root with more inodes than the default */
	err = sys_mount("tmpfs", "/root", "tmpfs", tmpfs_root_mountflags,
	    "nr_inodes=4096");
	
	/* The default root dir perms aren't really suitable for a root
	 * filesystem. */
	sys_chmod("/root",(mode_t)(S_IRUSR | S_IWUSR | S_IXUSR | S_IRGRP |
				S_IXGRP | S_IROTH | S_IXOTH));

	if (err) {
		printk(KERN_ERR "VFS: unable to mount tmpfs root filesystem.\n");
		return 1;
	}
...
#ifdef CONFIG_RG_ROOTFS_TMPFS
	if (!mount_tmpfs_root() && !prepare_non_block_rootfs())
	    goto out;
#endif
	if (mount_initrd) {
 ...
 */
static uch __init fill_inbuf(void)
{
