...
void do_IRQ(int irq, struct pt_regs * regs)
{
	...
	do {
		...
		/* Acknowledge interrupt after it is handled to avoid double
		 * interrupt cases. This should clear interrupt status of all
		 * intermediate units between IRQ source and IC
		 * (e.g. GPIO controller which keeps interrupt line active even
		 * after IRQ source is cleared)
		 */
		desc->ack(irq);
...
void __init init_IRQ(void)
{
	...
	for (irq = 0; irq < NR_IRQS; irq++) {
		...
		irq_desc[irq].ack      = dummy_mask_unmask_irq;
