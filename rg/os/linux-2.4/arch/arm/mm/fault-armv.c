 ...
 */
static int
do_sect_fault(unsigned long addr, int error_code, unsigned int fsr,
	struct pt_regs *regs)
{
	struct task_struct *tsk = current;
	do_bad_area(tsk, tsk->active_mm, addr, error_code, fsr, regs);
	return 0;
}
 ...
 */
static int
do_bad(unsigned long addr, int error_code, unsigned int fsr, 
    	struct pt_regs *regs)
{
	return 1;
}
...
static struct fsr_info {
	int (*fn)(unsigned long addr, int error_code, unsigned int fsr, 
		struct pt_regs *regs);
	int sig;
	const char *name;
} fsr_info[] = {
...
void __init
hook_fault_code(int nr, int (*fn)(unsigned long, int, unsigned int, 
    struct pt_regs *), int sig, const char *name)
{
 ...
 */
asmlinkage void
do_DataAbort(unsigned long addr, int error_code, struct pt_regs *regs, 
    	unsigned int fsr)
{
	...
	if (!inf->fn(addr, error_code, fsr, regs))
		return;
...
{
	do_translation_fault(addr, 0, 0, regs);
}
