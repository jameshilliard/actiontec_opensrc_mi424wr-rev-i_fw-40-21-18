		...
		unsigned int fsr, int code, struct pt_regs *regs)
{
	...
	printk(KERN_ERR "%s: unhandled page fault at pc=0x%08lx, "
	       "lr=0x%08lx (bad address=0x%08lx, code %d)\n",
	       tsk->comm, regs->ARM_pc, regs->ARM_lr, addr, fsr);
	show_regs(regs);
	dump_mem("User-mode stack:", regs->ARM_sp, TASK_SIZE);
#endif
...
do_bad_area(struct task_struct *tsk, struct mm_struct *mm, unsigned long addr,
	    int error_code, unsigned int fsr, struct pt_regs *regs)
{
	/*
...
#ifdef CONFIG_DEBUG_USER
	printk(KERN_ERR "%s: sigbus at 0x%08lx, pc=0x%08lx\n",
	...
	struct pt_regs *regs)
{
	...
	if (addr < TASK_SIZE)
		return do_page_fault(addr, error_code, fsr, regs);
