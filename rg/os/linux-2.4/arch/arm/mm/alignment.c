...
#ifndef CONFIG_ALIGNMENT_TRAP_MODE
#define CONFIG_ALIGNMENT_TRAP_MODE 5
#endif
static int ai_usermode = CONFIG_ALIGNMENT_TRAP_MODE;
...
static int
do_alignment(unsigned long addr, int error_code, unsigned int fsr, 
	struct pt_regs *regs)
{
 ...
 bad_or_fault:
	 ...
	 */
	do_bad_area(current, current->mm, addr, error_code, fsr, regs);
	return 0;
