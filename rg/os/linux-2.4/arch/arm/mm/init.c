...
#if defined(CONFIG_ARCH_IXP425) && defined(CONFIG_PCI) && (MEM_SIZE >= SZ_64M)
	{
		/* Exclude last physical page of PCI window
		 * from potential allocs because of IXP4XX PCI BUG */
		unsigned long bad_dma_start = SZ_64M - PAGE_SIZE;
		unsigned long bad_dma_size = PAGE_SIZE;
		printk("Memory range %#lx-%#lx is excluded "
			"because of PCI DMA bug\n",
			bad_dma_start, bad_dma_start + bad_dma_size - 1);
		reserve_bootmem_node(pgdat, bad_dma_start, bad_dma_size);
	}
#endif
}
