 ...
 *
 * $Log: h323ep.cxx,v $
 * Revision 1.5  2006/04/26 08:41:08  olegy
 * B31918 - new openh323 version (1_18_0)
 *
 *
 * Revision 1.2.2.2  2006/04/16 16:11:59  olegy
                                 ...
                                 const PString & localAddress)
{
  ...
  if (address.IsEmpty()) {
    ...
    if (identifier.IsEmpty())
      ...
      return SetGatekeeperZone(address, identifier, PString(), transport);
  }
}
...
BOOL H323EndPoint::SetGatekeeperZone(const PString & address,
                                     const PString & identifier,
                                     const PString & alternate_address,
                                     H323Transport * transport)
{
  H323Gatekeeper * gk = InternalCreateGatekeeper(transport);
  BOOL discoveryComplete = gk->DiscoverByNameAndAddress(identifier, address);

  if (!discoveryComplete && !alternate_address.IsEmpty())
    discoveryComplete = gk->DiscoverByNameAndAddress(PString(), alternate_address);
  return InternalRegisterGatekeeper(gk, discoveryComplete);
}
