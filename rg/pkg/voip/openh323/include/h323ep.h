 ...
 *
 * $Log: h323ep.h,v $
 * Revision 1.4  2006/04/26 08:40:30  olegy
...
class H323EndPoint : public PObject
{
  ...
  //@{
    ...
    BOOL SetGatekeeperZone(
      ...
      const PString & alternate_address,/// Address of alternate gatekeeper.
      H323Transport * transport = NULL  /// Transport over which to talk to gatekeeper.
    );
  ...
  //@{
    ...
    /**Set the default time for gatekeeper to reregister.
       A value of zero disables the keep alive facility.
     */
    void SetGatekeeperTimeToLive(int seconds) { registrationTimeToLive.SetInterval(0, seconds); }
