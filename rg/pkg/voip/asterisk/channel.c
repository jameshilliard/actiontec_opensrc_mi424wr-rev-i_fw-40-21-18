...
#ifdef ZAPTEL_OPTIMIZATIONS
	tmp->timingfd = gpl_sys_rg_chrdev_open(KOS_CDT_JTIMER, O_RDWR);
	if (tmp->timingfd > -1) {
		/* Check if timing interface supports new
		   ping/pong scheme */
		flags = 1;
		if (!ioctl(tmp->timingfd, JTIMER_TIMERPONG, &flags))
			needqueue = 0;
	}
#else
	tmp->timingfd = -1;					
...
int ast_queue_frame(struct ast_channel *chan, struct ast_frame *fin)
{
	...
	if (chan->alertpipe[1] > -1) {
		...
		ioctl(chan->timingfd, JTIMER_TIMERPING, &blah);
#endif				
...
int ast_settimeout(struct ast_channel *c, int samples, int (*func)(void *data), void *data)
{
	...
	if (c->timingfd > -1) {
		...
		res = ioctl(c->timingfd, JTIMER_TIMERCONFIG, &samples);
		c->timingfunc = func;
		c->timingdata = data;
	}
#endif	
...
struct ast_frame *ast_read(struct ast_channel *chan)
{
	...
	if ((chan->timingfd > -1) && (chan->fdno == AST_MAX_FDS - 2) && ast_test_flag(chan, AST_FLAG_EXCEPTION)) {
		...
		res = ioctl(chan->timingfd, JTIMER_GETEVENT, &blah);
		if (res) 
			blah = JTIMER_EVENT_TIMER_EXPIRED;

		if (blah == JTIMER_EVENT_TIMER_PING) {
#if 0
...
#endif				
				if (ioctl(chan->timingfd, JTIMER_TIMERPONG, &blah)) {
					ast_log(LOG_WARNING, "Failed to pong timer on '%s': %s\n", chan->name, strerror(errno));
				}
			}
		} else if (blah == JTIMER_EVENT_TIMER_EXPIRED) {
			ioctl(chan->timingfd, JTIMER_TIMERACK, &blah);
			...
			if (func) {
				...
				blah = 0;
				ast_mutex_lock(&chan->lock);
				ioctl(chan->timingfd, JTIMER_TIMERCONFIG, &blah);
...
/* Don't rely on incoming packets for sending packets because in many cases
 * incoming packets won't reach here */
#if 0
...
int ast_channel_make_compatible(struct ast_channel *chan, struct ast_channel *peer)
{
	...
	if (chan->readformat == chan->nativeformats && chan->ptime)
