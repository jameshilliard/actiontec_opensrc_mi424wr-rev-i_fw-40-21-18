 ...
 *
 * Version Info: $Id: ast_h323.cpp,v 1.17 2006/05/08 09:23:26 olegy Exp $
 */
#include <arpa/inet.h>
...
H323_REGISTER_CAPABILITY(H323_G722Capability, OPAL_G722_64k);
H323_REGISTER_CAPABILITY(H323_G726Capability, OPAL_G726_32k);

H323_G722Capability::H323_G722Capability()
  : H323AudioCapability(24, 2)
{
}

PObject * H323_G722Capability::Clone() const
{
    return new H323_G722Capability(*this);
}

PString H323_G722Capability::GetFormatName() const
{
  	return OPAL_G722_64k;
}

unsigned H323_G722Capability::GetSubType() const
{
  	return H245_AudioCapability::e_g722_64k;
}

H323Codec * H323_G722Capability::CreateCodec(H323Codec::Direction direction) const
{
  	return NULL;
}
...
H323_G726Capability::H323_G726Capability()
  : H323NonStandardAudioCapability(240, 80, (const BYTE *)&(OpalG726_32k), 
      strlen(OpalG726_32k), 0, strlen(OpalG726_32k))
{
}

PObject * H323_G726Capability::Clone() const
{
  	return new H323_G726Capability(*this);
}

unsigned H323_G726Capability::GetSubType() const
{
  	return H245_AudioCapability::e_nonStandard;
}

PString H323_G726Capability::GetFormatName() const
{
  	return OPAL_G726_32k;
}

H323Codec * H323_G726Capability::CreateCodec(H323Codec::Direction direction) const
{
  	return NULL;
}
...
{
	unsigned options = 0;
	call_options_t *opts = o ? (call_options_t *)o : &h323_options;
...
void h323_set_tcp_ports(int tcpstart, int tcpend)
{
	endPoint->SetTCPPorts(tcpstart, tcpend);
}
...
/** Establish Gatekeeper communiations, if so configured, 
  ...
  */
int h323_set_gk(int gatekeeper_discover, char *gatekeeper, char *id,
    char *secret, char *alternate_gk)
{
	PString gkName = PString(gatekeeper);
	PString gkID = PString(id);
	PString altGkName = PString(alternate_gk);
	...
	if (gatekeeper_discover) {
		...
		if (endPoint->SetGatekeeperZone(gkName, gkID, altGkName, rasChannel)) {
			cout << "== Using " << (endPoint->GetGatekeeper())->GetName() << " as our Gatekeeper." << endl;
		} else {
...
/** Check gatekeeper registation status
  */
int h323_is_registered_with_gk(void)
{
	return endPoint->IsRegisteredWithGatekeeper();
}

/** Set the gatekeeper time to live
  */
void h323_set_gk_time_to_live(int seconds)
{
	endPoint->SetGatekeeperTimeToLive(seconds);
}
