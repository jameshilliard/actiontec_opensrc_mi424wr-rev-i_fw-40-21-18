...
static struct ast_channel *__oh323_new(struct oh323_pvt *pvt, int state, const char *host)
{
	...
	if (ch) {
		...
		if (pvt->options.capability)
		    ch->nativeformats = ast_codec_choose(&pvt->options.prefs, pvt->options.capability, 1);
		else
		    ch->nativeformats = ast_codec_choose(&pvt->options.prefs, h323_options.capability, 1);
