...
#define DEFAULT_CALLERID "Anonymous"
	...
	if (pkt->retrans < MAX_RETRANS) {
		...
		pkt->retrans++;
 		if (!pkt->timer_t1) {	/* Re-schedule using timer_a and timer_t1 */
			...
			if (pkt->method == SIP_INVITE && pkt->retrans == MAX_RETRANS)
...
static int sip_call(struct ast_channel *ast, char *dest, int timeout)
{
	...
	if ( res != -1 ) {
		 ...
		 * chan_jdsp.c */
		if (pbx_builtin_getvar_helper(p->owner,"FORCECODEC"))
			p->capability = p->prefcodec;
...
static int sip_indicate(struct ast_channel *ast, int condition)
{
	...
	case AST_CONTROL_FAX:
		/* I am sending a fax. Should tell other side to switch to faxtxcodec (either PCMU or PCMA) */
		if (ast->readformat != p->faxtxcodec || ast->writeformat != p->faxtxcodec)
		{
		    p->capability = p->faxtxcodec;
		    transmit_reinvite_with_sdp(p);
		}
		break;
...
static struct ast_channel *sip_new(struct sip_pvt *i, int state, char *title)
{
	...
	tmp->ptime = ast_get_ptime_by_format(&i->prefs, tmp->nativeformats);
	tmp->tech_pvt = i;
...
static int sip_register(char *value, int lineno)
{
	...
	if (ast_strlen_zero(username) || ast_strlen_zero(hostname)) {
		ast_log(LOG_WARNING, "Format for registration is user[:secret[:authuser]]@host[:port][|outbound_proxy[:port]][/contact] at line %d\n", lineno);
		return -1;
	}
	stringp=username;
...
static int process_sdp(struct sip_pvt *p, struct sip_request *req)
{
	...
	if (!(p->owner->nativeformats & p->jointcapability)) {
		...
		ast_set_read_format(p->owner, p->owner->readformat);
		ast_set_write_format(p->owner, p->owner->writeformat);
		p->owner->ptime = ast_get_ptime_by_format(&p->prefs, p->owner->nativeformats);
	}

	/* Avoid timing problems in transfer scenario when this function
	 * called before bridging was done */
...
#if defined(T38_SUPPORT)
	if (sdp && (p->udptl) && ((p->t38state == 1) || (p->t38state == 2))) {
		ast_udptl_offered_from_local(p->udptl, 1);
...
static int transmit_refer(struct sip_pvt *p, struct sip_pvt *target_pvt, const char *dest)
{
	...
	if (c) {
	    if (target_pvt)
	    {
			char *callid = get_header(&target_pvt->initreq, "Call-ID");
			char fromtag[128];
			...
			snprintf(referto, sizeof(referto), "<%s?Replaces=%s%%3Bfrom-tag%%3D%s%%3Bto-tag%%3D%s>", destination, callid, fromtag, target_pvt->theirtag);
	    }
	    else {
...
static int get_refer_info(struct sip_pvt *sip_pvt, struct sip_request *outgoing_req)
{
	...
	if (!ast_strlen_zero(replace_callid)) {	
		...
		if ((sip_pvt_ptr = get_sip_pvt_byid_locked(replace_callid, req, replaces_callid_totag, replaces_callid_fromtag))) {
			...
			if ((ichan = sip_request_call("SIP", sip_pvt->owner ? sip_pvt->owner->readformat : AST_FORMAT_ULAW, refer_uri, &cause))) {
				struct ast_frame *f;
				char *rbuf;	
	...
	*t = '\0';
	...
	if (!user) {
		...
		if (sipmethod == SIP_SUBSCRIBE)
			...
			/* JUNGO: For invite, match peer name taken from the "To" field */
			char *tmp, to[256];
			...
			ast_copy_string(to, tmp, sizeof(to));
			tmp = get_in_brackets(to);
...
#define FORMAT  "%-30.30s  %-12.12s  %8d %-20.20s\n"
	...
	{
	        len += snprintf(out + len, sizeof(out)-len, FORMAT2, "Host", "Username", "Refresh", "State");
	}
	...
	ASTOBJ_CONTAINER_TRAVERSE(&regl, 1, do {
		...
		{
		        len += snprintf(out + len, sizeof(out)-len, FORMAT, host, iterator->username, iterator->refresh, regstate2str(iterator->regstate));
		}
		ASTOBJ_UNLOCK(iterator);
	} while(0));
			...
			if (p->t38state == 4) {
				...
				ast_log(LOG_DEBUG,"T38 state changed to %d on channel %s\n",p->t38state, p->owner ? p->owner->name : "<none>");
					...
					if ((bridgepeer=ast_bridged_channel(p->owner))) {
						...
						if (bridgepvt && bridgepvt->t38state == 5) {
								ast_log(LOG_WARNING, "RTP re-inivte after T38 session not handled yet !\n");
								/* Insted of this we should somehow re-invite the other side of the bridge to RTP */
								if (ignore)
...
static struct sip_peer *build_peer(const char *name, struct ast_variable *v, int realtime)
{
	...
	peer->callgroup = 0;
	peer->faxtxcodec = AST_FORMAT_ULAW;
	peer->pickupgroup = 0;
	peer->rtpkeepalive = global_rtpkeepalive;
	...
	while(v) {
		...
		} else if (!strcasecmp(v->name, "faxtxcodec")) {
		    if (!strcasecmp(v->value, "ulaw"))
			peer->faxtxcodec = AST_FORMAT_ULAW;
		    else if (!strcasecmp(v->value, "alaw"))
			peer->faxtxcodec = AST_FORMAT_ALAW;
		}
		/* else if (strcasecmp(v->name,"type"))
		 *	ast_log(LOG_WARNING, "Ignoring %s\n", v->name);
		 */
