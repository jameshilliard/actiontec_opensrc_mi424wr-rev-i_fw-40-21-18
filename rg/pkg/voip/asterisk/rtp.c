...
static struct ast_frame *send_dtmf(struct ast_rtp *rtp, int type)
{
	...
	if ((type == AST_FRAME_DTMF_BEGIN) && ast_tvcmp(ast_tvnow(), rtp->dtmfmute) < 0) {
		if (option_debug)
...
void ast_rtp_set_formats(struct ast_rtp *rtp, int readformat, int writeformat)
{
	...
	if (readformat)
	{
	    params.media_payload_type = params.rtp_payload_type =
		ast_rtp_lookup_code(rtp, 1, readformat);
	    if (ioctl(rtp->fd, RTP_PAYLOAD, &params))
	    {
			ast_log(LOG_ERROR, "Unable to set jrtp's current read payload %d\n",
		    	params.rtp_payload_type);
			return;
	    }
	}
	...
	{
	    params.media_payload_type = params.rtp_payload_type =
		ast_rtp_lookup_code(rtp, 1, writeformat);
	    if (ioctl(rtp->fd, RTP_PAYLOAD, &params))
	    {
			ast_log(LOG_ERROR, "Unable to set jrtp's current write payload %d\n",
		    	params.rtp_payload_type);
			return;
	    }
	}
...
void ast_rtp_set_peer(struct ast_rtp *rtp, struct sockaddr_in *them)
{
	...
	if (ioctl(rtp->fd, RTP_MODE, RTP_SENDRECEIVE))
	{
...
char *ast_rtp_get_quality(struct ast_rtp *rtp)
{
	...
	*/
	...
	snprintf(rtp->rtcp->quality, sizeof(rtp->rtcp->quality), "ssrc=%u;"
	    "themssrc=%u;lp=%u;lpr=%u;rxjitter=%u;rxcount=%u;rxoctets=%u;"
	    "txjitter=%u;txcount=%u;txoctets=%u;rlp=%u;rlpr=%u;rtt=%u", 0, 0,
	    stats.rx_packets_lost, stats.rx_loss_rate, stats.rx_jitter,
	    stats.rx_packets, stats.rx_octets, stats.reported_jitter,
	    stats.tx_packets, stats.tx_octets, stats.reported_packets_lost,
	    stats.reported_loss_rate, stats.round_trip_delay);
...
int ast_rtp_senddigit_begin(struct ast_rtp *rtp, char digit)
{
	...
	if (ioctl(rtp->fd, RTP_TIMESTAMP_GET, &timestamp))
	{
...
static int ast_rtp_raw_write(struct ast_rtp *rtp, struct ast_frame *f, int codec)
{
	...
	int pred;
	int mark = 0;
	u32 oldts = rtp->lastts;
	u32 olddigitts = rtp->lastdigitts;
	u32 oldovid = rtp->lastovidtimestamp;
	...
	*/
	...
	if (rtp->them.sin_port && rtp->them.sin_addr.s_addr) {
		...
		if (res <0) {
		    	/* Undo update of some variables if write failed */
		    	rtp->lastovidtimestamp = oldovid;
		    	rtp->lastdigitts = olddigitts;
		    	rtp->lastts = oldts;
