...
static struct ast_channel *wait_for_answer(struct ast_channel *in, struct localuser *outgoing, int *to, struct ast_flags *peerflags, int *sentringing, char *status, size_t statussize, int busystart, int nochanstart, int congestionstart, int priority_jump, int *result, struct ast_channel **bridged_to)
{
	struct localuser *o;
	int found;
	int numlines;
...
static int dial_exec_full(struct ast_channel *chan, void *data, struct ast_flags *peerflags)
{
	...
	struct ast_channel *peer, *bridged_to = NULL;
	int to;
	int numbusy = 0;
	int numcongestion = 0;
	int numnochan = 0;
	...
	time(&start_time);
	peer = wait_for_answer(chan, outgoing, &to, peerflags, &sentringing, status, sizeof(status), numbusy, numnochan, numcongestion, ast_test_flag(&opts, OPT_PRIORITY_JUMP), &result, &bridged_to);
	
	/* Close the bridge that was opened in wait_for_answer */
	if (bridged_to)
		ast_channel_post_bridge(chan, bridged_to);
