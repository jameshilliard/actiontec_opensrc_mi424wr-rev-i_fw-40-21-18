...
/* XXX Needs fixing due to change of ast_codec_pref definition */
#if 0
...
{
	int slot = 0;
	int size = sizeof(pref->order) / sizeof(struct ast_codec_pref_item);

	
	if((index >= 0) && (index < size)) {
		slot = pref->order[index].slot;
	}
...
int ast_codec_pref_append(struct ast_codec_pref *pref, int format, int ptime)
{
	size_t size = 0;
	int x = 0, newindex = -1;
	...
	if(newindex) {
		for (x = 0; x < size; x++) {
			if(!pref->order[x].slot) {
				pref->order[x].slot = newindex;
				pref->order[x].ptime = ptime;
				break;
			}
		}
	}
...
{
	char *parse;
	char *this, *ptime_str = NULL;
	int format, ptime;

	parse = ast_strdupa(list);
	while ((this = strsep(&parse, ","))) {
 		if ((ptime_str = strchr(this, ':'))) {
 			*ptime_str = '\0';
 			ptime_str++;
 		}
		...
		ptime = ptime_str ? atoi(ptime_str) : 0;
		if (pref) {
			if (strcasecmp(this, "all")) {
				if (allowing)
					ast_codec_pref_append(pref, format, ptime);
				else
					ast_codec_pref_remove(pref, format);
			} else if (!allowing) {
