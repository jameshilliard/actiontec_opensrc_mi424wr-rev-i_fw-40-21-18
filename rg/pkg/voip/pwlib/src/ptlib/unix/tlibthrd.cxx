 ...
 *
 * $Log: tlibthrd.cxx,v $
 * Revision 1.6  2006/04/26 10:16:21  olegy
 * B31918 - importing new 1.10.0 pwlib, 1.18.0 openh323 versions
 *
 * Revision 1.5  2005/12/25 12:18:53  tali
 * B27344 After a call from a UA with which OpenRG has no common codec, FXS cannot make or receive more calls
 *
 * Revision 1.4  2005/12/14 10:52:38  itay
 * AUTO MERGE: 1 <- branch-4_1
 * B26090 fix crash which results from a mutex unlock without it being locked
 * (a PSemaphore is constructed and then immediately destructed)
 * NOTIFY: automerge
 *
 * Revision 1.3.2.1  2005/12/14 10:52:33  itay
 * B26090 fix crash which results from a mutex unlock without it being locked
 * (a PSemaphore is constructed and then immediately destructed)
 *
 ...
 *
 * Revision 1.150.2.2  2006/03/01 08:34:33  csoutheren
 * Backported patch #1439578 from CVS head
 *
 * Revision 1.150.2.1  2006/01/29 23:34:52  csoutheren
...
#if !( (defined(P_NETBSD) || defined(__linux__)) && defined(P_NO_CANCEL) )
    pthread_testcancel();
#endif
  }
}
...
BOOL PTimedMutex::Wait(const PTimeInterval & waitTime) 
{
  ...
  if (waitTime == PMaxTimeInterval) {
    ...
    /* The uClibc implementation of pthread_mutex_timedlock() ignores the timeout
     * it is given, for certain mutex types, and performs infinite lock instead.
     * For zero waitTime, bypass this problem by calling trylock. */
