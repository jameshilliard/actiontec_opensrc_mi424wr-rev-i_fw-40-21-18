 ...
 *
 * $Log: svcproc.cxx,v $
 * Revision 1.7  2007/02/15 09:37:35  shayh
 * B32363 - CLI part.
 * - Remove old logging CLI commands and add the new ones according to
 *   pkg/doc/design/multitask_log.txt.
 * - Make the firewall log unavailable if there is not firewall in the DIST.
 * - Fix double console printing in rg_error() if log level is set (once from
 *   rg_error_register() callback, and once by calling console_printf()).
 * - Separation mt_cli.[ch] (put it in pkg/cli/main).
 * - Create a separate entity for telnet CLI sessions (ENT_TELNETS_CLI).
 * - Also fix a bug in CLI - if you logout from boot menu, then you reach
 *   the login prompt, which is incorrect behavior.
 * Also fix WBM issues related to B32363:
 * - Make secondary sort of time descending instead of ascending.
 * - Fix sorting of details column (it couldn't be sorted)
 * Also fix bug in logging of external processes:
 * - openlog() doesn't duplicate the char *ident argument, so we can't pass
 *   an automatic stack buffer to it - we need a dynamically allocated space.
 *   Therefore, I couldn't use an inline function anymore. I separated
 *   log_entity.[ch] into two main parts - JGPL and JUNGO license.
 *
 * Revision 1.6  2007/02/07 12:08:58  shayh
 * B32363 - WBM part.
 * - Also fix the syslog engine by propagting the entity ID through the different
 *   layers.
 * - Also use a single and common API for adding sorted table headers
 *   (common to Web-CIFS, QoS utilization and now also the syslog table).
 * - Also fix a bug - the buttons in the syslog page were placed too low - they
 *   need to be placed higher. This is because of a p_break() call that shouldn't
 *   have been invoked in that case.
 * - Also change "Save" to "Download" and "Load" to "Upload" in the WBM and
 *   when naming variables.
 *
 * Revision 1.5  2007/02/05 09:09:54  shayh
 * B42703 - remove redundant component prefix from rg_error messages
 *
 * Revision 1.4  2006/04/26 10:16:21  olegy
 ...
 *
 * Revision 1.2.2.2  2006/04/16 11:41:23  olegy
...
int PServiceProcess::InitialiseService()
{
  ...
  if (systemLogFileName.IsEmpty())
    rg_openlog((char *)(const char *)GetName(), LOG_PID, LOG_DAEMON);
