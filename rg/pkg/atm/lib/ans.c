...
static int ans(const char *text,int wanted,void *result,int res_len)
{
    ...
    int name_len,data_len,found_len;
	...
	return TRY_OTHER;
#else
    fprintf(stderr, "can't use res_search\n");
    return TRY_OTHER;
#endif
	...
	return TRY_OTHER;
#else
    fprintf(stderr, "can't use dn_expand\n");
    return TRY_OTHER;
#endif
    pos += name_len;
    if (GET16(pos) != wanted || GET16(pos+2) != C_IN)
	return TRY_OTHER;
    pos += 4;
    /*
  ...
  int flags)
{
    ...
    if (addr->sas_addr.prv) {
        ...
        if (!res && !ans(tmp,T_PTR,buffer,length))
	    return 0;
