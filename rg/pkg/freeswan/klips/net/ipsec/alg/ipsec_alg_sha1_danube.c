 ...
 * 
 *  rg/pkg/freeswan/klips/net/ipsec/alg/ipsec_alg_sha1_danube.c
 *
...
typedef struct {
    u8 key_ipad[SHA1_HMAC_BLOCK_SIZE];
    u8 key_opad[SHA1_HMAC_BLOCK_SIZE];
} sha1_hmac_danube_context_t;
...
extern void danube_sha1_init(void *ctx);
extern void danube_sha1_update (void *ctx, const u8 *data, unsigned int len);
extern void danube_sha1_final (void *ctx, u8 *out);

#define DANUBE_AUTH_DEBUG
#ifdef DANUBE_AUTH_DEBUG
#define DEBUG_PRINT(fmt, p...) \
    printk("auth debug(sha1): %s:" fmt "\n", __FUNCTION__, ##p)
#else
#define DEBUG_PRINT(fmt, p...)
#endif

static int _sha1_hmac_danube_set_key(struct ipsec_alg_auth *alg, __u8 *key_a,
    const __u8 *key, int key_len) 
{
    sha1_hmac_danube_context_t *ctx = (sha1_hmac_danube_context_t *)key_a;
    int i;
	...
	danube_sha1_init(&tmp);
	danube_sha1_update(&tmp, key, key_len);
	danube_sha1_final(&tmp, key);

	key_len = SHA1_HMAC_DIGEST_SIZE;
    }
...
static int _sha1_hmac_danube_hash(struct ipsec_alg_auth *alg, __u8 *key_a,
    const __u8 *data, int data_len, __u8 *hash, int hash_len) 
{
    ...
    danube_sha1_init(&tmp);
    danube_sha1_update(&tmp, ctx->key_ipad, sizeof(ctx->key_ipad));
    danube_sha1_update(&tmp, data, data_len);
    danube_sha1_final(&tmp, digest);

    /* HASH((key XOR opad) HASH((key XOR ipad) text)) */
    danube_sha1_init(&tmp);
    danube_sha1_update(&tmp, ctx->key_opad, sizeof(ctx->key_opad));
    danube_sha1_update(&tmp, digest, sizeof(digest));
    danube_sha1_final(&tmp, digest);

    /* clip if needed */
    if (hash_len >= SHA1_HMAC_DIGEST_SIZE)
...
static struct ipsec_alg_auth ipsec_alg_SHA1_danube = {
    ixt_version:	IPSEC_ALG_VERSION,
    ixt_module:		THIS_MODULE,
    ixt_refcnt:		ATOMIC_INIT(0),
    ixt_name:		"sha1_danube",
    ...
    ixt_keymaxbits:	160,
    ixt_a_keylen:	160/8,
    ixt_a_ctx_size:	sizeof(sha1_hmac_danube_context_t),
    ixt_a_hmac_set_key:	_sha1_hmac_danube_set_key,
    ixt_a_hmac_hash:	_sha1_hmac_danube_hash,
};

IPSEC_ALG_MODULE_INIT(ipsec_sha1_init)
{
    ipsec_log("Danube-ipsec driver : register SHA1 hash algorithm\n");
    return register_ipsec_alg_auth(&ipsec_alg_SHA1_danube);
}

IPSEC_ALG_MODULE_EXIT(ipsec_sha1_fini)
{
    unregister_ipsec_alg_auth(&ipsec_alg_SHA1_danube);
}
