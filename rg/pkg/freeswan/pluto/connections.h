...
struct connection {
    char *name;
    lset_t policy, openrg_quick_policy, openrg_main_policy;
...
/* note: this macro only applys to any-address templates: kind == CK_INSTANCE */
#define his_id_was_instantiated(c) ((c)->kind == CK_INSTANCE \
    && (id_is_ipaddr(&(c)->that.id)? \
    sameaddr(&(c)->that.id.ip_addr, &(c)->that.host_addr) : TRUE))

#define IS_INSTANCE_KIND(k) ((k) == CK_INSTANCE || (k) == CK_INSTANCE_NET)
#define IS_INSTANCE(c) IS_INSTANCE_KIND((c)->kind)
...
extern void delete_connection_instance(char *conn_name, unsigned long instance);
