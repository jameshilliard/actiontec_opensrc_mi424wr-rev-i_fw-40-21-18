...
#ifdef NO_SECRET_FILE
void ipc_get_secrets(void);
#endif
...
/* Convert ip to id. Zero ip is a %any analog.
 * IPV6 is not supported.
 */
static int make_secret_address(struct secret *s, ip_address *ip)
{
    struct id id;
    err_t ugh = NULL;

    if (!ip->u.v4.sin_addr.s_addr)
    {
	id = empty_id;
	id.kind = ID_IPV4_ADDR;
	ugh = anyaddr(AF_INET, &id.ip_addr);
    ...
    {
	struct id_list *i = alloc_thing(struct id_list, "id_list");
...
/* Function get following values:
 *   - ipc connection fd;
 *   - internal struct to store gotten private key.
 */
static int ipc_get_rsa_private_key_file(int fd, struct secret *s)
{
    char *filename, *passphrase = NULL;
    pkcs1privkey_t *key = NULL;
    err_t ugh = NULL;
    int rc = -1;
    u_int i;

    if (!(filename = ipc_string_read(fd)) ||
	!(passphrase = ipc_string_read(fd)))
    {
	goto Exit;
    }
    key = load_pkcs1_private_key(filename, passphrase);
    ...
    if (!key)
    {
	loglog(RC_LOG_SERIOUS, "error loading RSA private key file \"%s\"",
	    filename);
	goto Exit;
    }

    for (i = 0; i < elemsof(RSA_private_field); i++)
    {
	MP_INT *n = (MP_INT *) ((char *)&s->u.RSA_private_key +
	    RSA_private_field[i].offset);
...
/* Read RSA secrets. They must be sent by order:
 * modulus, public exponent, private exponent, prime1, prime2,
 * exponent1, exponent2, coefficient.
 *
 * Each value is sent as string.
 */
static int ipc_get_rsa_secret(int fd, struct secret *s)
{
    char buf[2048];	/* limit on size of binary representation of key */
    const struct fld *p;
    char *data = NULL;
    struct RSA_private_key *rsak = &s->u.RSA_private_key;
    err_t ugh = NULL;
    ...
    for (p=RSA_private_field; p<&RSA_private_field[elemsof(RSA_private_field)];
	p++)
    {
	size_t sz;
	err_t ugh;
	...
	/* Last data will be freed on exit. */
	free(data);
	if (!(data = ipc_string_read(fd)))
	{
	    ugh = "ipc read fail";
	    goto Exit;
	}
	if ((ugh = ttodatav(data, strlen(data), 0, buf, sizeof(buf),
	    &sz, diag_space, sizeof(diag_space))))
	{
	    /* in RSA key, ttodata didn't like */
	    ugh = builddiag("RSA data malformed (%s): %s", ugh, p->name);
	    goto Exit;
	}
	...
	if (pb_next < &pub_bytes[elemsof(pub_bytes)])
	{
	    if (eb_next - ebytes + sz > sizeof(ebytes))
	    {
		ugh = "public key takes too many bytes";
		goto Exit;
	    }
	    setchunk(*pb_next, eb_next, sz);
	    memcpy(eb_next, buf, sz);
    ...
    bits = mpz_sizeinbase(&rsak->pub.n, 2);

    rsak->pub.k = (bits + BITS_PER_BYTE - 1) / BITS_PER_BYTE;
    rsak->pub.keyid[0] = '\0';	/* in case of splitkeytoid failure */
    splitkeytoid(pub_bytes[1].ptr, pub_bytes[1].len , pub_bytes[0].ptr,
	pub_bytes[0].len , rsak->pub.keyid, sizeof(rsak->pub.keyid));
...
/* Function gets two types of data:
 * 1. RSA secret (exponents, modulus, etc.):
 * 2. Private certificate and its password.
 *
 * Return value: 0 - success, -1 - fail.
 */
static int ipc_get_rsa(int fd, struct secret *s)
{
    int data_type, rc;

    if (ipc_read(fd, (u8 *)&data_type, sizeof(data_type)))
	return -1;
    
    s->kind = PPK_RSA;
    switch (data_type)
    {
    case IPSEC_PPK_RSA_PRIVATE_KEY_FILE:
	rc = ipc_get_rsa_private_key_file(fd, s);
	break;
    case IPSEC_PPK_RSA_SECRET:
	rc = ipc_get_rsa_secret(fd, s);
	break;
    default:
	return -1;
    }
...
/* Function get shared secret data. It get following values:
 * - source IP,
 * - destination IP,
 * - shared secret.
 * Return value: 0 - success, -1 - fail.
 ...
 * IPV6 is not supported.
 */
static int ipc_get_shared_secret(int fd, struct secret *s)
{
    ip_address src, dst;
    char *secret;

    src.u.v4.sin_family = AF_INET;
    dst.u.v4.sin_family = AF_INET;
    if (ipc_read(fd, (u8 *)&src.u.v4.sin_addr.s_addr,
	sizeof(src.u.v4.sin_addr.s_addr)) ||
	ipc_read(fd, (u8 *)&dst.u.v4.sin_addr.s_addr,
	sizeof(dst.u.v4.sin_addr.s_addr)) ||
	!(secret = ipc_string_read(fd)))
    {
	return -1;
    }
    s->kind = PPK_PSK;
    make_secret_address(s, &src);
    make_secret_address(s, &dst);
    clonetochunk(s->u.preshared_secret, secret, strlen(secret), "PSK");
    free(secret);
    return 0;
}
...
void ipc_get_secrets(void)
{
    int rc = -1, fd = -1;
    int kind;
    ipsec_proc_cmd_t cmd = IPSEC_CMD_SECRETS_REQUEST;
    struct secret *s;
    ...
    while (!(rc = ipc_read(fd, (u8 *)&kind, sizeof(kind))) &&
	kind!=IPSEC_PPK_NONE)
    {
	s = (struct secret *)alloc_thing(struct secret, "secret");
