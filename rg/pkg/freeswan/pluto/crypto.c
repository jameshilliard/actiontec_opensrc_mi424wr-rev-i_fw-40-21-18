...
struct encrypt_desc oakley_encrypter[OAKLEY_CAST_CBC + 1] = {
    /* (none) */
	{ 0, 0, NULL },
    /* OAKLEY_DES_CBC */
...
struct hash_desc oakley_hasher[OAKLEY_TIGER+1] = {
...
{
	return &oakley_hasher[alg];
}
struct encrypt_desc *crypto_get_encrypter(int alg)
{
	return &oakley_encrypter[alg];
}
