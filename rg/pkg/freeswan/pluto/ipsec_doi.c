...
static const struct oakley_group_desc *openrg_pfsgroup_get(lset_t policy,
#if !defined(NO_IKE_ALG) && !defined(NO_KERNEL_ALG)
    struct state *st,
#else
    struct connection *c,
#endif
    const struct oakley_group_desc *ike_group)
{
#if !defined(NO_IKE_ALG) && !defined(NO_KERNEL_ALG)
    struct oakley_group_desc const *group = NULL;

    if (st->st_connection)
	    group = ike_alg_pfsgroup(st->st_connection, policy);
    if (!group)
	    group = policy & POLICY_PFS? ike_group : NULL;
    return group;
#else
    if (policy & POLICY_PFS)
    {
	if (c->openrg_quick_policy & 
	    (OPENRG_MODP_768 | OPENRG_MODP_1024 | OPENRG_MODP_1536))
	{
	    return lookup_group(aggr_get_oakley_policy(rg_oakley_group,
		c->openrg_quick_policy &
		(OPENRG_MODP_768 | OPENRG_MODP_1024 | OPENRG_MODP_1536)));
	}
	else
	    return ike_group;
...
, const struct oakley_group_desc *group, pb_stream *outs, u_int8_t np)
{
	...
	*g = mpz_to_n(&mp_g, group->bytes);
	...
	if (g == &st->st_gr)
	{
	    compute_dh_shared(st, st->st_gi, group);
	    if (st->st_shared.ptr[0] == 0)
	    {
		/* generate a new secret to avoid this situation */
		loglog(RC_LOG_SERIOUS, "regenerating DH private secret to avoid Pluto 1.0 bug"
...
, pb_stream *pbs)
{
    ...
    {
	DBG_reject_loglog(RC_LOG_SERIOUS, "KE has %u byte DH public value; %u required",
 ...
 */
void
send_ipsec_delete(struct state *st)
{
    pb_stream reply_pbs;
    pb_stream r_hdr_pbs;
    msgid_t	msgid;
...
, int is_pending_phase2)
{
    ...
    if (c->dpd_delay)
    {
	/* DPD VID is sent if it is enabled for this connection. */
	numvidtosend++;
    }
#ifdef NAT_TRAVERSAL
    if (nat_traversal_enabled)
	 ...
	 */
	int np = (c->this.id.kind == ID_KEY_ID)? ISAKMP_NEXT_VID :
	    (numvidtosend-- > 0 ? ISAKMP_NEXT_VID : ISAKMP_NEXT_NONE);

	oakley_sadb = oakley_sadb_alloc(c->openrg_main_policy,
	    auth_policy >> POLICY_ISAKMP_SHIFT);
	if (!oakley_sadb)
	{
	    reset_cur_state();
	    return STF_INTERNAL_ERROR;
	}
	if (!out_sa(&rbody, oakley_sadb	, st, TRUE, FALSE, np))
	{
	    cur_state = NULL;
	    sadb_free(&oakley_sadb);
	    return STF_INTERNAL_ERROR;
	}
	sadb_free(&oakley_sadb);
	...
	int is_pending_phase2)
{
	...
	oakley_sadb = oakley_sadb_alloc(c->openrg_main_policy, POLICY_PSK);
	if (!oakley_sadb ||
	    !init_st_oakley(st, c->openrg_main_policy) ||
	    !out_sa(&rbody, oakley_sadb, st, TRUE, TRUE, ISAKMP_NEXT_KE))
	{
	    reset_cur_state();
	    sadb_free(&oakley_sadb);
	    return STF_INTERNAL_ERROR;
	...
	id_hd.isaiid_np = numvidtosend-- > 0 ? ISAKMP_NEXT_VID : ISAKMP_NEXT_NONE;
...
, struct state *old_state)
{
    ...
    if (st == NULL)
    {
	if (old_state && old_state->delete_sa)
	{
	    /* We got delete quick state message as part of disconnect
	     * process: delete main state was received later. So, no need
	     * for new negotiation.
	     */
	    return;
	}
	else
	{
	    ...
	    (void) initiator(whack_sock, c, NULL, policy, try, 1);
	}
    }
    else
    {
...
ipsecdoi_replace(struct state *st, unsigned long try)
{
	...
	(void) initiator(whack_sock, st->st_connection, st, policy, try, 0);
    }
    else
    {
...
Exit:
    DBG(DBG_IKE_REJECT, log_reject("authentication failed"));
    return FALSE;
}
...
, union hash_ctx *ctx
#ifndef NO_IKE_ALG
, void (*hash_update_func)(void *, const u_char *input, size_t))
#else
...
#define HASH_UPDATE_T (union hash_ctx *, const u_char *input, unsigned int len)
    hash_update_t hash_update=(hash_update_t)  hash_update_func;
#endif
...
#define CHECK_QUICK_HASH(md, do_hash, hash_name, msg_name) { \
	...
	{ \
	    ...
	    DBG_reject_loglog(RC_LOG_SERIOUS, "received " hash_name " does not match computed value in " msg_name); \
	    /* XXX Could send notification back */ \
	    return STF_FAIL + INVALID_HASH_INFORMATION; \
	} \
    }
...
accept_nonce(struct msg_digest *md, chunk_t *dest, const char *name)
{
    ...
    {
	DBG_reject_loglog(RC_LOG_SERIOUS, "%s length not between %d and %d"
 ...
 */
static bool
emit_phase2_id(struct end *end_point
, u_int8_t np, u_int8_t protoid, u_int16_t port, pb_stream *outs, int is_tunnel)
{
    ...
    size_t tal;
    const struct af_info *af_inf;
    int is_client = is_tunnel && end_point->has_client;

    id.isaiid_np = np;
    if (!(af_inf = aftoinfo(is_client ?
	subnettypeof(&end_point->client) : AF_INET)))
    {
	return FALSE;
    }
    id.isaiid_idtype = !is_client ? af_inf->id_addr :
	(end_point->client.is_subnet ? af_inf->id_subnet : af_inf->id_range);
    ...
    ta = is_client ? *SUBNET_ADDR_GET(&end_point->client) :
	end_point->host_addr;
    tal = addrbytesptr(&ta, &tbp);
    if (!out_raw(tbp, tal, &id_pbs, !is_client ? "endpoint IP address" :
	(end_point->client.is_subnet ? "client network" :
	"client range start")))
    {
	return FALSE;
    }

    /* In transport mode and tunnel mode GW-to-GW phase 2 ID would be host_addr
     * only without netmask (length 4 octets). In this case RFC doesn't demand
     * to send the ID, but it is needed for Windows and Safenet
     * interoperability.
     * In tunnel mode with subnet/range the phase 2 ID length is 8 octets
     * (subnet/netmask or startIP/endIP).
     */
    if (is_client)
    {
	if (end_point->client.is_subnet)
	    maskof(&end_point->client, &ta);
	else
	    ta = end_point->client.v.range.to;
	tal = addrbytesptr(&ta, &tbp);
	if (!out_raw(tbp, tal, &id_pbs,
	    (end_point->client.is_subnet ? "client mask" : "client range end")))
	{
	    return FALSE;
	}
    }
...
, so_serial_t replacing)
{
	...
	*r_hash_start;	/* start of what is to be hashed */
    ...
    /* if PFS is specified and there is no openrg-policy for group of phase 2,
     * use the same group as during Phase 1: since no negotiation is possible,
     * we pick one that is very likely supported.
     */
    st->st_pfs_group = openrg_pfsgroup_get(policy,
#if !defined(NO_IKE_ALG) && !defined(NO_KERNEL_ALG)
	st,
#else
	c,
#endif
	isakmp_sa->st_oakley.group);

    /* Original code always use all supported propositions, but we can enable or
     * disable them. So, instead static array we allocate memory for
     * propositions and add only the enabled propositions (signed by
     * openrg_quick_policy flag).
     * POLICY_COMPRESS is considered iff we can do IPcomp.
     */
    out_sadb = ipsec_sadb_alloc(c->openrg_quick_policy,
	can_do_IPcomp && (st->st_policy & POLICY_COMPRESS));
    if (!out_sadb)
    {
	reset_cur_state();
	return STF_INTERNAL_ERROR;
    }
    if (!out_sa(&rbody, out_sadb, st, FALSE, FALSE, ISAKMP_NEXT_NONCE))
    {
	sadb_free(&out_sadb);
	reset_cur_state();
	return STF_INTERNAL_ERROR;
    ...
    , policy & POLICY_PFS? ISAKMP_NEXT_KE : ISAKMP_NEXT_ID
    , "Ni"))
    {
	reset_cur_state();
	return STF_INTERNAL_ERROR;
...
decode_peer_id(struct msg_digest *md, bool initiator, bool aggrmode)
{
    ...
    && !(id->isaid_doi_specific_a == IPPROTO_UDP &&
    (id->isaid_doi_specific_b == IKE_UDP_PORT || id->isaid_doi_specific_b == 0)))
    {
	loglog(RC_LOG_SERIOUS, "protocol/port in Phase 1 ID Payload must be "
	    "0/0 or %d/%d or %d/0 but are %d/%d"
	    , IPPROTO_UDP, IKE_UDP_PORT, IPPROTO_UDP
	    , id->isaid_doi_specific_a, id->isaid_doi_specific_b);
	return FALSE;
    }
...
main_inI1_outR1(struct msg_digest *md)
{
	...
	if (!c)
	{
	    DBG(DBG_IKE_REJECT, log_reject("initial Main Mode message "
		"received on %s:%u but no connection has been authorized",
    ...
    {
	log_reject("initial Main Mode message received on %s:%u but can only "
	    "accept Aggressive mode.", ip_str(&md->iface->addr), pluto_port);
	return STF_IGNORE;
    }
    ...
    if ((c->kind == CK_INSTANCE) && (c->that.host_port_specific))
    {
    ...
    next = numvidtosend-- ? ISAKMP_NEXT_VID : ISAKMP_NEXT_NONE;
    if (c->dpd_delay && !out_vid(next, &md->rbody, VID_MISC_DPD))
      return STF_INTERNAL_ERROR;

#ifdef NAT_TRAVERSAL
    DBG(DBG_CONTROL, DBG_log("sender checking NAT-t: %d and %d"
				, nat_traversal_enabled
...
main_inR1_outI2(struct msg_digest *md)
{
	...
	st->hidden_variables.st_nat_traversal = nat_traversal_vid_to_method(md->quirks.nat_traversal_vid);
	log("enabling possible NAT-traversal with method %s"
...
report_key_dns_failure(struct id *id, err_t ugh)
{
    ...
    DBG_reject_loglog(RC_LOG_SERIOUS, "no RSA public key known for '%s'"
...
, cont_fn_t cont_fn)	/* continuation function */
{
    ...
    {
	DBG(DBG_IKE_REJECT, log_reject("invalid ID information"));
...
aggr_inI1_outR1(struct msg_digest *md)
{
	...
	c = find_host_connection(&md->iface->addr, pluto_port
	    , (ip_address*)NULL, md->sender_port);
	if (!c || !(c->policy & POLICY_AGGRESSIVE))
	{
	    DBG_reject_loglog(RC_LOG_SERIOUS, "initial Aggressive "
		"Mode message from %s but no (wildcard) connection "
		"has been configured", ip_str(&md->sender));
	    /* XXX notification is in order! */
	    return STF_IGNORE;
	}
    }
    ...
    if (!(c->policy & POLICY_AGGRESSIVE))
    {
	log_reject("initial Aggressive Mode message received on %s:%u but can "
	    "only accept Main mode.", ip_str(&md->iface->addr), pluto_port);
	return STF_IGNORE;
    }
    
    if (c->kind == CK_TEMPLATE && c->policy & POLICY_AGGRESSIVE)
    {
	/* Create a temporary connection that is a copy of this one.
	 * His ID isn't declared yet.
	 */
    ...
    st->st_is_full_phase1_id_payload =
	(bool) md->chain[ISAKMP_NEXT_ID]->payload.id.isaid_doi_specific_a;
	...
	next_payload = numvidtosend-- > 0 ? ISAKMP_NEXT_VID : ISAKMP_NEXT_NONE;
	if (auth_payload == ISAKMP_NEXT_HASH)
	{
	    /* HASH_R out */
	...
	else
	{
	    ...
	    if (!out_generic_raw(next_payload, &isakmp_signature_desc
	    , &md->rbody, sig_val, sig_len, "SIG_R"))
		return STF_INTERNAL_ERROR;
	}
    }
    ...
    /* send DPD VID */
    if (c->dpd_delay && !out_vid(next_payload, &md->rbody, VID_MISC_DPD)) {
	return STF_INTERNAL_ERROR;
    }
...
aggr_inI2(struct msg_digest *md)
{
	...
	if (st->st_is_full_phase1_id_payload)
	{
	    ...
	    id_hd.isaiid_port = IKE_UDP_PORT;
	}
...
report_verify_failure(struct state *st, struct p2id *his, err_t ugh)
{
    ...
    subnettot(&his->net, 0, cb, sizeof(cb));
...
, const u_char new_iv[MAX_DIGEST_LEN])
{
	...
	*r_hash_start;	/* from where to start hashing */
	...
	{
	    p = c;
	    DBG(DBG_CONTROL, DBG_log("using old for transport mode connection '%s'", p->name));
	}
#endif
	if (p == NULL)
	{
	...
	else if (p != c)
	{
		...
		if (p->policy & POLICY_OPPO)
		{
		    ...
		    our_client = *SUBNET_ADDR_GET(our_net);
		    his_client = *SUBNET_ADDR_GET(his_net);
	...
	if ((st->st_policy & POLICY_PFS) &&
	    (st->st_pfs_group!=openrg_pfsgroup_get(st->st_policy,
#if !defined(NO_IKE_ALG) && !defined(NO_KERNEL_ALG)
	    st,
#else
	    c,
	    ...
	    st->st_oakley.group)))
	{
	    loglog(RC_LOG_SERIOUS, "we got invalid PFS group for phase 2");
	    return STF_FAIL + NO_PROPOSAL_CHOSEN;
	}
...
#ifdef NAT_TRAVERSAL
#define DONT_CHECK_OUR_ID \
	((st->hidden_variables.st_nat_traversal & NAT_T_DETECTED) \
	&& (!(st->st_policy & POLICY_TUNNEL)) \
	&& (st->hidden_variables.st_nat_traversal & LELEM(NAT_TRAVERSAL_NAT_BHND_ME)))
#else
#define DONT_CHECK_OUR_ID 0
#endif
	    if (!DONT_CHECK_OUR_ID
	    && !check_net_id(&id_pd->payload.ipsec_id, &id_pd->pbs
	    , &st->st_myuserprotoid, &st->st_myuserport
	    , &st->st_connection->this.client
	    , "our client"))
		return STF_FAIL + INVALID_ID_INFORMATION;
#undef DONT_CHECK_OUR_ID
     ...
     * failure won't look like success.
     */
    old_serial = st->st_connection->newest_ipsec_sa;
    st->st_connection->newest_ipsec_sa = st->st_serialno;
    if (!install_ipsec_sa(st, TRUE))
    {
	st->st_connection->newest_ipsec_sa = old_serial;
	return STF_INTERNAL_ERROR;
    }
    st->st_connection->newest_ipsec_sa = old_serial;
