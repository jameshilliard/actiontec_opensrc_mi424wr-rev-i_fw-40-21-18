...
void release_old_states(struct connection *c)
{
	...
	for (st = statetable[i]; st != NULL; )
	{
	    ...
	    if (this->st_connection==c &&
		this->st_serialno!=c->newest_isakmp_sa &&
		this->st_serialno!=c->newest_ipsec_sa &&
		((IS_PHASE1(this->st_state) && this->st_serialno < c->newest_isakmp_sa) ||
		(IS_QUICK(this->st_state) && this->st_serialno < c->newest_ipsec_sa)))
	    {
		struct state *old_cur_state
		    = cur_state == this? NULL : cur_state;
			     ...
			     , struct connection *c)
{
    ...
    st->st_localaddr  = c->this.host_addr;
    st->st_localport  = c->this.host_port;
    st->st_remoteaddr = c->that.host_addr;
    st->st_remoteport = c->that.host_port;
