...
help(void)
{
	...
	"deletestate: whack"
	    ...
	    "\n\n"
	"deleteinstance: whack"
	    " --deleteinstance <instance_number>"
	    "\n\n"
	"pubkey: whack"
	    " --keyid <id>"
	    " [--addkey]"
...
#   define OPT_LAST OPT_BLOCK_IP	/* last "normal" option */
...
#   define CD_POLICY_FIRST  CD_PSK
    ...
    CD_PFSGROUP,
    CD_ESP,
    CD_OPENRG_MAIN_POLICY,
    CD_OPENRG_QUICK_POLICY
#   define CD_LAST CD_OPENRG_QUICK_POLICY /* last connection description */
...
#   define DBGOPT_FIRST DBGOPT_NONE
    ...
    DBGOPT_IKE_REJECT,	/* same order as DBG_* */
    DBGOPT_PRIVATE,	/* same order as DBG_* */
    DBGOPT_IKE_LOG_ALL,	/* same order as DBG_* */
#   define DBGOPT_LAST DBGOPT_IKE_LOG_ALL
#endif
...
#   define OO	OPTION_OFFSET
    ...
    { "deleteinstance", required_argument, NULL, OPT_DELETEINSTANCE + OO + NUMERIC_ARG },
    ...
    { "listevents", no_argument, NULL, OPT_LISTEVENTS + OO },
    ...
    { "anti_replay", required_argument, NULL, OPT_ANTI_REPALY + OO },
    { "block_ip", required_argument, NULL, OPT_BLOCK_IP + OO },
    ...
    { "clientrange", required_argument, NULL, CD_CLIENTRANGE + OO },
    ...
    { "openrg_main_policy", required_argument, NULL,
	CD_OPENRG_MAIN_POLICY + OO + NUMERIC_ARG },
    { "openrg_quick_policy", required_argument, NULL,
	CD_OPENRG_QUICK_POLICY + OO + NUMERIC_ARG },
    ...
    { "debug-ike-reject", no_argument, NULL, DBGOPT_IKE_REJECT + OO },
    { "debug-log-all", no_argument, NULL, DBGOPT_IKE_LOG_ALL + OO },
#endif
#   undef OO
    { 0,0,0,0 }
};
...
static void
update_address_port(ip_address * a, sa_family_t family, u_int16_t port)
{
    switch (family) {
    case AF_INET:
        a->u.v4.sin_port = port;
        break;
    case AF_INET6:
        a->u.v6.sin6_port = port;
        break;
    }
}

static void update_net_port(ip_subnet *range, sa_family_t fam, u_int16_t port)
{
    if (range->is_subnet)
	update_address_port(&range->v.net.addr, fam, port);
    else
    {
	update_address_port(&range->v.range.from, fam, port);
	update_address_port(&range->v.range.to, fam, port);
    }
}
...
update_ports(struct whack_message * m)
{
    ...
    if (m->left.port != 0) {
        ...
        update_address_port(&m->left.host_addr, m->addr_family, port);
        update_net_port(&m->left.client, m->tunnel_addr_family, port);
    }
    if (m->right.port != 0) {
        port = htons(m->right.port);
        update_address_port(&m->right.host_addr, m->addr_family, port);
        update_net_port(&m->right.client, m->tunnel_addr_family, port);
    }
}
...
, unsigned long long policy, bool default_nexthop, sa_family_t caf
, sa_family_t taf)
{
    ...
    if (this->has_client)
    {
	ip_address cn = *SUBNET_ADDR_GET(&this->client);
...
main(int argc, char **argv)
{
	...
	opts_seen = LEMPTY,
	cd_seen = LEMPTY,
	cd_seen_before_to = LEMPTY; /* init to avoid warning */
    const char
	*af_used_by = NULL,
	*tunnel_af_used_by = NULL;
	...
	else if (CD_FIRST <= c && c <= CD_LAST)
	{
	     ...
	     * Reject repeated options (unless later code intervenes).
	     */
	    unsigned long long f = LELEM(c - CD_FIRST);
	...
	case OPT_DELETEINSTANCE: /* --deleteinstance <instance_number> */
	    msg.whack_deleteinstance = TRUE;
	    msg.whack_deleteinstanceno = opt_whole;
	    continue;
	...
	case OPT_LISTEVENTS:	/* --listevents */
	...
	case OPT_ANTI_REPALY:
	    msg.whack_anti_replay = TRUE;
	    msg.anti_replay_enabled = strcmp(optarg, "off");
	    continue;
	...
	case OPT_BLOCK_IP:
		...
		msg.whack_block_ip = TRUE;
		af_used_by = long_opts[long_index].name;
		/* argument format is: <reject_num>:<block_period> */
		msg.block_ip_reject_num = strtol(optarg, &end_ptr, 10);
		end_ptr++;
		msg.block_ip_period = strtol(end_ptr, NULL, 10);
		continue;
	    }
	...
	case CD_CLIENT:	/* --client <subnet> */
		...
		LELEM(CD_CLIENTRANGE - CD_FIRST)))
	    {
		diag("--client conflicts with --clientwithin or --clientrange");
	    }
	...
	case CD_CLIENTWITHIN:	/* --clienwithin <address range> */
		...
		LELEM(CD_CLIENTRANGE - CD_FIRST)))
	    {
		diag("--clientwithin conflicts with --client or --clientrange");
	    }
	    tunnel_af_used_by = long_opts[long_index].name;
	    diagq(ttosubnet(optarg, 0, msg.tunnel_addr_family, &msg.right.client), optarg);
	    msg.right.has_client = TRUE;
	...
	case CD_CLIENTRANGE:
	    if (cd_seen & (LELEM(CD_CLIENT - CD_FIRST) |
		LELEM(CD_CLIENTWITHIN - CD_FIRST)))
	    {
		diag("--clientrange conflicts with --client or --clientwithin");
	    }
	    tunnel_af_used_by = long_opts[long_index].name;
	    diagq(ttorange(optarg, 0, msg.tunnel_addr_family,
		&msg.right.client), optarg);
	    msg.right.has_client = TRUE;
	    msg.policy |= POLICY_TUNNEL;
	    continue;
	...
	case CD_AGGRESSIVE:	/* --aggrmode */
	    msg.policy |= POLICY_AGGRESSIVE;
	    continue;

	case CD_OPENRG_MAIN_POLICY:
	    msg.openrg_main_policy = opt_whole;
	    continue;

	case CD_OPENRG_QUICK_POLICY:
	    msg.openrg_quick_policy = opt_whole;
	    continue;
	...
	case DBGOPT_IKE_REJECT: /* --debug-ike-reject */
	case DBGOPT_IKE_LOG_ALL:/* --debug-log-all */
    ...
    | LELEM(OPT_DELETE) | LELEM(OPT_CD) | LELEM(OPT_DELETEINSTANCE)))
    {
    ...
    || msg.whack_delete || msg.whack_deletestate || msg.whack_deleteinstance
    ...
    || msg.whack_anti_replay || msg.whack_block_ip))
    {
