...
#define AES128_CBC_KEY_SIZE	(128 / BITS_PER_BYTE)
#define AES192_CBC_KEY_SIZE	(192 / BITS_PER_BYTE)
#define AES256_CBC_KEY_SIZE	(256 / BITS_PER_BYTE)
...
#define DBG_DPD		LELEM(8)
#define DBG_NATT	LELEM(9)
#define DBG_IKE_REJECT	LELEM(10)	/* rejected IKE packets */
#define DBG_PRIVATE	LELEM(11)	/* private information: DANGER! */
#define DBG_LOG_ALL	LELEM(12)	/* print all log messages (ignore rate limit) */

#define DBG_NONE	0		/* no options on */
#define DBG_ALL		LRANGES(DBG_RAW, DBG_IKE_REJECT)	/* all options on EXCEPT DBG_PRIVATE */
#endif
...
enum connection_kind {
    ...
    CK_INSTANCE_NET,	/* instance of template, created for a particular
			 * attempt, when the remote address is known in the
			 * template - the template allows any remote subnet for
			 * a specific ip */
...
#define POLICY_AGGRESSIVE    LELEM(12)
 ...
 * This macro does not check shunt policy, because in OpenRG a connection
 * always has PSK or RSASIG policy.
 */
