...
init_ctl_socket(void)
{
    ...
    /* notify main-task that pluto's control socket is open */
    ipc_cmd = IPSEC_CMD_CTRL_SK_OPEN;
    if (!failed && ((ipc_fd = ipc_connect(htons(RG_IPC_PORT_IPSEC))) < 0 ||
	(rc = ipc_write(ipc_fd, (char *)&ipc_cmd, sizeof(ipc_cmd)))))
    {
	failed = "notify main-task of the openning of the";
    }
    if (ipc_fd >= 0)
	ipc_client_close(ipc_fd, rc);
...
process_raw_ifaces(struct raw_iface *rifaces)
{
	...
	for (vfp = rifaces; vfp != NULL; vfp = vfp->next)
	{
		...
		{
		    int vfp_is_ppp = !strncmp(vfp->name, PPPDEVPREFIX,
			sizeof(PPPDEVPREFIX)-1);

		    /* If one of them is a PPP device - this is probably
		     * unnumbered PPP so prefer it.
		     */
		    if (v_is_ppp && !vfp_is_ppp)
			/* nothing */;
		    else if (!v_is_ppp && vfp_is_ppp)
		    {
			v = vfp;
			v_is_ppp = TRUE;
		    }
...
show_ifaces_status(void)
{
	...
	whack_log(RC_COMMENT, "interface %s/%s %s:%d"
	    , p->vname, p->rname, ip_str(&p->addr), p->port);
}
