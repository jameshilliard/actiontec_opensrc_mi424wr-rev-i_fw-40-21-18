...
void rsasigkey(int nbits, int useoldkey, char *keys[RSA_SEC_ALL]);
...
char *argv[];
{
	...
	ipsec_proc_cmd_t cmd = IPSEC_CMD_RSASIGKEY;
	char *keys[RSA_SEC_ALL];
	...
	rsasigkey(nbits, (oldkeyfile == NULL) ? 0 : 1, keys);
	for (i=PUBKEY; i<RSA_SEC_ALL; i++)
	{
	    if (!keys[i])
		goto Exit;
	}
	if ((fd = ipc_connect(htons(RG_IPC_PORT_IPSEC)))<0 ||
	    (ipc_write(fd, (char *)&cmd, sizeof(cmd))))
	{
	    fprintf(stderr, "%s: failed OpenRG ipc\n", me);
	    goto Exit;
	}
	for (i=PUBKEY; i<RSA_SEC_ALL; i++)
	{
	    if (ipc_string_write(fd, keys[i]))
	    {
		fprintf(stderr, "%s: failed OpenRG ipc\n", me);
		goto Exit;
	    }
 ...
 */
void
rsasigkey(nbits, useoldkey, keys)
int nbits;
...
char *keys[RSA_SEC_ALL];
{
	...
	bundp = bundle(E, n, &bs);
	keys[PUBKEY]  = strdup(conv(bundp, bs, 'x'));	/* RFC2537ish format */
	keys[MODULUS] = strdup(hexout(n));
	keys[PUBLICEXPONENT] = strdup(hexout(e));
	keys[PRIVATEEXPONENT] = strdup(hexout(d));
	keys[PRIME1] = strdup(hexout(p));
	keys[PRIME2] = strdup(hexout(q));
	keys[EXPONENT1] = strdup(hexout(exp1));
	keys[EXPONENT2] = strdup(hexout(exp2));
	keys[COEFFICIENT] = strdup(hexout(coeff));
}
