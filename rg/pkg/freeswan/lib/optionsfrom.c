...
static char *local_getline(FILE *, char *, size_t);
...
char **linep;			/* where to store pointer if successful */
{
	...
	while (w->pending == NULL) {	/* no pending line */
		if ((w->line = local_getline(f, w->buf, sizeof(w->buf))) == NULL)
	...
	if (w->pending == w->line && w->line[0] != '-') {
		...
		if (w->line == w->buf) {
			*linep = malloc(endp - p + 1);
			...
			strcpy(*linep, p);
		} else			/* local_getline already malloced it */
			*linep = p;
		return NULL;
	}
...
/*
 - local_getline - read a line from the file, trim newline off
 */
static char *			/* pointer to line, NULL for eof/error */
local_getline(f, buf, bufsize)
FILE *f;
