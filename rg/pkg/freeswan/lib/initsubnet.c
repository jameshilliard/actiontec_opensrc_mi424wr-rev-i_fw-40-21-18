...
ip_subnet *dst;
{
	...
	dst->is_subnet = 1;
	dst->v.net.addr = *addr;
	n = addrbytesptr(&dst->v.net.addr, (const unsigned char **)&p);
	if (n == 0)
...
ip_subnet *dst;
{
	...
	dst->is_subnet = 1;
	dst->v.net.addr = *addr;
	n = addrbytesptr(&dst->v.net.addr, (const unsigned char **)NULL);
	if (n == 0)
		return "unknown address family";
	dst->v.net.maskbits = n*8;
	return NULL;
}
