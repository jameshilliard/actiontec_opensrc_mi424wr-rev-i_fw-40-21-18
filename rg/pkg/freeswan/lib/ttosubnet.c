...
err_t ttorange(char *src, size_t srclen, int af, ip_subnet *dst)
{
	char *to;
	err_t err;
	ip_address tmp_from, tmp_to;

	if (!srclen && !(srclen = strlen(src)))
		return "empty string";

	if (!(to = memchr(src, '-', srclen)))
		return "no '-' in range specification";

	if ((err = ttoaddr(src, (size_t)(to-src), af, &tmp_from)))
		return err;
	to++;
	if ((err = ttoaddr(to, srclen-(to-src), af, &tmp_to)))
		return err;
	return initrange(&tmp_from, &tmp_to, dst);
}
...
#	define	DEFIS6LEN	(sizeof(defis6) - 1)
	...
	oops = ttoaddr(src, (size_t)(slash-src), af, &addrtmp);
	if (oops != NULL)
		return oops;
