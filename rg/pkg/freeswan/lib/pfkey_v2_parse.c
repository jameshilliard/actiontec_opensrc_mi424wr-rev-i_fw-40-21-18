...
char pfkey_v2_parse_c_version[] = "pfkey_v2_parse.c,v 1.6.8.1 2003/12/03 14:17:33 noams Exp";
...
#endif
# include "ipsec_log.h"  /* KLIPS_PRINT */
		...
		int dir)
{
	...
	while( (remain * IPSEC_PFKEYv2_ALIGN) >= sizeof(struct sadb_ext) ) {
		...
		if((extensions_seen & ( 1ULL << pfkey_ext->sadb_ext_type )) != 0)
		{
			DEBUGGING(PF_KEY_DEBUG_PARSE_PROBLEM,
				"pfkey_msg_parse: "
		...
		extensions_seen |= ( 1ULL << pfkey_ext->sadb_ext_type );
