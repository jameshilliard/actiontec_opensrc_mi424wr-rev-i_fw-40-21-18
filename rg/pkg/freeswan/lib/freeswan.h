 ...
 * logic loses track and swats them?  32 is the best KLIPS can do.
 */
...
#  undef DEBUG_NO_STATIC
#  define DEBUG_NO_STATIC static


#endif /* __KERNEL__ */

#ifndef CONFIG_RG_IPSEC_ESP_ALG
#define NO_KERNEL_ALG
#endif

#ifndef CONFIG_RG_IPSEC_IKE_ALG
#define NO_IKE_ALG
#endif

#ifdef CONFIG_IPSEC_ENC_1DES
#define USE_SINGLE_DES
#endif
...
#define SUBNET_ADDR_GET(subnet) \
    ((subnet)->is_subnet ? &(subnet)->v.net.addr : &(subnet)->v.range.from)
...
err_t ttorange(char *src, size_t srclen, int af, ip_subnet *dst);
...
#define	SUBNETTOT_BUF	(ADDRTOT_BUF*2 + 1)
...
err_t initrange(const ip_address *from, const ip_address *to, ip_subnet *dst);
...
#define KFREE_S(ptr, size) kfree(ptr)
#else
#define KFREE_S(ptr, size) kfree_s(ptr, size)
#endif

/*
 * jungo specific
 */
#ifndef PF_KEY /* old version of glibc */
#define PF_KEY 15
#endif
