...
const ip_subnet *src;
{
	return src->is_subnet ? src->v.net.addr.u.v4.sin_family :
	    src->v.range.from.u.v4.sin_family;
}
...
ip_address *dst;
{
	...
	size_t n = addrlenof(&src->v.net.addr);
	unsigned char *p;

	if (src->v.net.maskbits > n*8 || n > sizeof(buf))
		return;		/* "can't happen" */

	p = buf;
	for (b = src->v.net.maskbits; b >= 8; b -= 8)
		*p++ = 0xff;
	if (b != 0)
		*p++ = (0xff << (8 - b)) & 0xff;
	while (p - buf < n)
	...
	(void) initaddr(buf, n, addrtypeof(&src->v.net.addr), dst);
}
