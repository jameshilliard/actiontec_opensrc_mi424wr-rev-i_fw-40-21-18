...
#define USBLP_MSG_TIMEOUT	(HZ/10)			/* 0.1 seconds */
...
static int usblp_ctrl_msg(struct usblp *usblp, int request, int type, int dir, int recip, int value, void *buf, int len, int timeout)
{
	int retval = usb_control_msg(usblp->dev,
		dir ? usb_rcvctrlpipe(usblp->dev, 0) : usb_sndctrlpipe(usblp->dev, 0),
		request, type | dir | recip, value, usblp->ifnum, buf, len, timeout);
	dbg("usblp_control_msg: rq: 0x%02x dir: %d recip: %d value: %d len: %#x result: %d",
		request, !!dir, recip, value, len, retval);
	return retval < 0 ? retval : 0;
}

#define usblp_read_status(usblp, status, timeout)\
	usblp_ctrl_msg(usblp, USBLP_REQ_GET_STATUS, USB_TYPE_CLASS, USB_DIR_IN, USB_RECIP_INTERFACE, 0, status, 1, timeout)
#define usblp_get_id(usblp, config, id, maxlen)\
	usblp_ctrl_msg(usblp, USBLP_REQ_GET_ID, USB_TYPE_CLASS, USB_DIR_IN, USB_RECIP_INTERFACE, config, id, maxlen, USBLP_WRITE_TIMEOUT)
#define usblp_reset(usblp)\
	usblp_ctrl_msg(usblp, USBLP_REQ_RESET, USB_TYPE_CLASS, USB_DIR_OUT, USB_RECIP_OTHER, 0, NULL, 0, USBLP_WRITE_TIMEOUT)

#define usblp_hp_channel_change_request(usblp, channel, buffer) \
	usblp_ctrl_msg(usblp, USBLP_REQ_HP_CHANNEL_CHANGE_REQUEST, USB_TYPE_VENDOR, USB_DIR_IN, USB_RECIP_INTERFACE, channel, buffer, 1, USBLP_WRITE_TIMEOUT)
...
static int usblp_check_status(struct usblp *usblp, int err)
{
	...
	error = usblp_read_status (usblp, usblp->statusbuf, USBLP_WRITE_TIMEOUT);
	if (error < 0) {
...
static int usblp_ioctl(struct inode *inode, struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	if (_IOC_TYPE(cmd) == 'P')	/* new-style ioctl number */
		...
		switch (cmd) {
			...
			case LPGETSTATUS:
				if (usblp_read_status(usblp, usblp->statusbuf, USBLP_MSG_TIMEOUT)) {
...
{
	int i;
	unsigned int quirk = 0;

	for (i = 0; quirk_printers[i].vendorId; i++) {
		if (vendor == quirk_printers[i].vendorId &&
		    product == quirk_printers[i].productId) {
		    	quirk = quirk_printers[i].quirks;
			break;
		}
 	}
#ifdef USBLP_NO_BIDIR
	quirk |= USBLP_QUIRK_BIDIR;
#endif
	return quirk;
}
...
static int device_str_len(unsigned char *s)
{
    	return (s[0] << 8) | s[1];
}
...
static int usblp_cache_device_id_string(struct usblp *usblp)
{
	int err, length = device_str_len(usblp->device_id_string);
	 ...
	 * the user's buffer. */
	length = device_str_len(usblp->device_id_string);
	if (length < 2)
		length = 2;
