...
#define TERM_BIT __constant_cpu_to_le32(1)
...
qh_completions (struct ehci_hcd *ehci, struct ehci_qh *qh, struct pt_regs *regs)
{
...
#ifdef QTD_STATUS_WA
	current_completed = (qh->hw_qtd_next & TERM_BIT) && 
	    !(qh->hw_token & ACTIVE_BIT);
#endif
	...
	list_for_each_safe (entry, tmp, &qh->qtd_list) {
		...
		if (current_completed && 
		    (cpu_to_le32(qtd->qtd_dma) == qh->hw_current) &&
		    (qtd->hw_token & ACTIVE_BIT))
		{
...
#ifdef QTD_STATUS_WA
		if (current_completed && ((token & QTD_STS_ACTIVE) == 0)) {
#else
...
#ifdef QTD_STATUS_WA
 	if (current_completed && qh->hw_qtd_next != EHCI_LIST_END)
	{
	    qh->hw_qtd_next &= ~TERM_BIT;
	}
#endif
	return count;
}
 ...
 */
#ifdef QTD_STATUS_WA
#define QTD_NEXT_TERM(dma) (QTD_NEXT(dma)|TERM_BIT)
#else
#define QTD_NEXT_TERM(dma) QTD_NEXT(dma)
#endif
...
qh_urb_transaction (
	...
	if (usb_pipecontrol (urb->pipe)) {
		...
		if (unlikely (!qtd))
			goto cleanup;
		qtd->urb = urb;
		qtd_prev->hw_next = QTD_NEXT_TERM(qtd->qtd_dma);
		list_add_tail (&qtd->qtd_list, head);
	} 
