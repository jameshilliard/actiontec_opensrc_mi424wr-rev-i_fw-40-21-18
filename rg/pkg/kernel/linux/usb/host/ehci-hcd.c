...
#ifdef CONFIG_CX8620X_EHCI
	cx8620x_set_host_mode(ehci->regs);
#endif
	return retval;
}
...
#ifdef CONFIG_CX8620X_EHCI
		/* XXX: wait till FORCE PORT RESUME bit changes to 0 */
#else
		wait_ms (20);
		temp &= ~PORT_RESUME;
		writel (temp, &ehci->regs->port_status [i]);
...
{
#ifdef CONFIG_CX8620X_EHCI
	return cx8620x_hc_init((struct hc_driver *)&ehci_driver);
#else
