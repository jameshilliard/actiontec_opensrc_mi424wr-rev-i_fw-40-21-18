...
static int ohci_reboot (struct notifier_block *self, unsigned long code,
    void *null)
{
...
static struct notifier_block ohci_notifier = {
    ohci_reboot,
    NULL,
    1, /* Need higher priority than the EHCI reboot notifier */
};
...
{
	int rc = pci_module_init (&ohci_pci_driver);

	if (rc)
	    return rc;

	register_reboot_notifier (&ohci_notifier);
...
{	
	unregister_reboot_notifier (&ohci_notifier);
	pci_unregister_driver (&ohci_pci_driver);
}
