...
static void process_options(int argc, char * const argv[])
{
	...
	for (;;) {
		int option_index = 0;
		static const char *short_options = "bs:f:i:l:opqnNca";
		...
		switch (c) {
			...
			case 'f':
				if (!(dumpfile = strdup(optarg))) {
					perror("strdup dumpfile");
					exit(EXIT_FAILURE);
				}
				break;
			case 'i':
				if (!(inputfile = strdup(optarg))) {
					perror("strdup inputfile");
					exit(EXIT_FAILURE);
				}
				break;
			case 'l':
...
int main(int argc, char * const argv[])
{
	...
	bool eccstats = false;
#endif
#define MAX_MTD_BUF_SIZE 4096
	unsigned char ibuf[MAX_MTD_BUF_SIZE];
	...
	/* ACTION_TEC - Open input file for reading */
	if (inputfile) {
	    if ((ifd = open(inputfile, O_RDONLY, 0644))== -1) {
		perror(inputfile);
		goto closeall;
	    }
	    if (!(omitbad || omitoob)) {
		fprintf(stderr, "-o and -b option must be specified with -i <filename>\n");
		goto closeall;
	    }
	    if (!dumpfile) {
		fprintf(stderr, "-f <filename> option must be specified with -i <filename>\n");
		goto closeall;
	    }
	}
	...
	bs = mtd.min_io_size;
	if (inputfile) {
	    if (bs > MAX_MTD_BUF_SIZE) {
		fprintf(stderr, "Page size of NAND Flash is > %d\n", MAX_MTD_BUF_SIZE);
	    }
	}
	...
	for (ofs = start_addr; ofs < end_addr; ofs += bs) {
		...
		if (badblock) {
			...
			if (inputfile) {
			    ibytes = read(ifd, ibuf, bs);
			    if (ibytes == 0) {
				/* End-of-file */
				printf("********************************************************\n");
				printf("nanddump: uboot in flash is different (size mismatch)!!!\n");
				printf("********************************************************\n");
				goto closeall_ok;
			    }

			    if (memcmp(ibuf, readbuf, ibytes))
			    {
				printf("*****************************************\n");
				printf("nanddump: uboot in flash is different !!!\n");
				printf("*****************************************\n");
				goto closeall_ok;
			    }
			}
	...
	if (inputfile) {
	    printf("nanddump: uboot in flash matches %s\n", inputfile);
	}
closeall_ok:
