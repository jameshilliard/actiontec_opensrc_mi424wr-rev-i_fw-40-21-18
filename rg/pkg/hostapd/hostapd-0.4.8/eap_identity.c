...
#ifdef EAP_WSC
        printf("***********Check PEER ID started\n");
        if(strncmp((char *)pos,"WFA-SimpleConfig-Registrar",
                    strlen("WFA-SimpleConfig-Registrar"))==0)
        {
                printf("***********WSC_PEER_ID_REG received flag=%x\n",
                                               sm->sta->flags);
                wsc_event_notify("WSC_PEER_ID_REG");
                sm->sta->flags |= WLAN_STA_WSC_REG_ID_RCVD;
        } else if(strncmp((char *)pos,"WFA-SimpleConfig-Enrollee",
                    strlen("WFA-SimpleConfig-Enrollee"))==0)
        {
                printf("*********** WFA-SimpleConfig-Enrollee received flag=%x\n",
                          sm->sta->flags);
                sm->sta->flags |= WLAN_STA_WSC_STA_ID_RCVD;
                wsc_event_notify("WSC_PEER_ID_STA");
        }
#endif
