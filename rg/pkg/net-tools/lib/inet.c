...
static int INET_resolve(char *name, struct sockaddr_in *sin, int hostfirst)
{
...
#else
    fprintf(stderr, "Your C library doesn't have getnetbyname()\n");
    np =  (struct netent *) NULL;
#endif
    if (np != (struct netent *) NULL) {
	...
	errno = h_errno;
#else
	fprintf(stderr, "Your C library doesn't have h_errno\n");
	errno = ENOSYS;
#endif
	return -1;
    }
...
#else
	fprintf(stderr, "Your C library doesn't have getnetbyaddr()\n");
	np = NULL;
#endif
