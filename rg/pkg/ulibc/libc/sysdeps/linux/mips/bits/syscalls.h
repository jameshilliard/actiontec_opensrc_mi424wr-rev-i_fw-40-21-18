...
/* TODO B51534 - this temporary change (casting of arg1 and arg2 to typeof
 * instead of ARG_TYPE) is needed for resolving a problem with the syscall
 * setregid in MIPS64 platform (CN3XXX). The problem is that syscall receives
 * two arguments - "unsigned int" type, but if the value of any of them is "-1",
 * then for 64-bit platform after casting for ARG_TYPE their values will be
 * garbled (0x00000000ffffffff instead of 0xffffffffffffffff)
 */
...
({									\
	...
	{								\
	register ARG_TYPE __v0 __asm__("$2") ncs_init;			\
	register typeof(arg1) __a0 __asm__("$4") = arg1;		\
	register typeof(arg2) __a1 __asm__("$5") = arg2;		\
	register ARG_TYPE __a3 __asm__("$7");				\
	__asm__ __volatile__ (						\
	".set\tnoreorder\n\t"						\
	cs_init								\
