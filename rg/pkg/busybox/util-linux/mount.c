...
#if 0
			{
				char default_codepage[24];
			
				if (!strcmp(string_flags, "")) {
					memset(default_codepage, 0, sizeof(default_codepage));
					snprintf(default_codepage, sizeof(default_codepage)-1, 
							"iocharset=cp%s", CONFIG_RG_CODEPAGE);
					paste_str(&string_flags, default_codepage);
				}
			}
#endif
