...
#		define PRELOAD_ENV_SZ sizeof("LD_PRELOAD")
    		static char preload_buf[PRELOAD_SZ + PRELOAD_ENV_SZ];
		...
		old_preload = getenv ("LD_PRELOAD");
		  ...
		  strcpy (preload_buf, "LD_PRELOAD=");
		  strcat (preload_buf, old_preload);
	        }
		pipe = popen(cmd_buf, "r");
		if (0 != old_preload
		    && 0 != putenv (preload_buf)) {
