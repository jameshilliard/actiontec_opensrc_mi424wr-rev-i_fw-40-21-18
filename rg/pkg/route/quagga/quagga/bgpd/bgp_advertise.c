...
bgp_advertise_intern (struct hash *hash, struct attr *attr)
{
  ...
  baa = (struct bgp_advertise_attr *) hash_get (hash, &ref, (void * (*) (void *))baa_hash_alloc);
  baa->refcnt++;
...
bgp_sync_init (struct peer *peer)
{
  ...
  for (afi = AFI_IP; afi < AFI_MAX; afi++)
	...
	peer->hash[afi][safi] = hash_create ((unsigned int (*) (void *))baa_hash_key, (int (*) (void *, void *))baa_hash_cmp);
      }
}
