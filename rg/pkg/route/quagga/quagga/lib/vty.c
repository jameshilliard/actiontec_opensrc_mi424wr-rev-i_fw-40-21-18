...
static void
vty_read_file (FILE *confp, int dryrun)
{
  int ret;
  struct vty *vty;
  ...
  if ( !((ret == CMD_SUCCESS) || (ret == CMD_ERR_NOTHING_TODO)) ) 
    {
      switch (ret)
       {
         case CMD_ERR_AMBIGUOUS:
           fprintf (dryrun ? stdout : stderr, "Ambiguous command.\n");
           break;
         case CMD_ERR_NO_MATCH:
           fprintf (dryrun ? stdout : stderr, "There is no such command.\n");
           break;
       }
      fprintf (dryrun ? stdout : stderr, "Error occured during reading below line:\n%s\n", vty->buf);
